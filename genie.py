...
def set_pv(name, value, wait=False, is_local=False):
    ...
    if not isinstance(value, int):
        ...
        try:
            if __api.pv_exists(name + '.ZNAM'):
                if get_pv(name + '.ZNAM').upper() == value.upper():
                    found = True
                    intValue = 0
                elif get_pv(name + '.ONAM').upper() == value.upper():
                    found = True
                    intValue = 1
            elif __api.pv_exists(name + '.ZRST'):
                for index, stringValue in enumerate(mbbiArray):
                    if get_pv(name + '.' + stringValue).upper() == value.upper():
                        found = True
                        intValue = index
                        break
        except Exception:
    ...
    try:
        if found:
            ...
            print name + " SET TO: " + str(value.upper())
        else:
