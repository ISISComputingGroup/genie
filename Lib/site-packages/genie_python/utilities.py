...
## so you need to keep returned handle always in scope and let it get cleaned up only
## on process termination by the natural built in resource cleanup system
...
class EnvironmentDetails(object):
    ...
    def get_settings_directory(self):
        default_directory = "C:/Instrument/Settings/config/{}/configurations".format(self._host_name)
