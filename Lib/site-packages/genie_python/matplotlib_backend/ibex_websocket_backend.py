...
def _ignore_if_websocket_closed(func):
    ...
    def wrapper(*a, **kw):
        try:
            ...
            try:
                GenieLogger().log_info_msg(f"Caught (non-fatal) exception while calling matplotlib function: "
...
def _asyncio_send_exceptions_to_logfile_only(loop, context):
    ...
    try:
        GenieLogger().log_info_msg(f"Caught (non-fatal) asyncio exception: "
...
asyncio.get_event_loop().set_exception_handler(_asyncio_send_exceptions_to_logfile_only)
    ...
    def pyplot_show(cls, *args, **kwargs):
        ...
        if not WebAggApplication.started:
            with IBEX_BACKEND_LOCK:
                ...
                worker_thread = threading.Thread(target=WebAggApplication.start, daemon=True)
                worker_thread.start()
    ...
    def new_figure_manager(cls, num, *args, **kwargs):
        with IBEX_BACKEND_LOCK:
            ...
            if len(figure_numbers) > max_number_of_figures:
                ...
                print(f"There are too many figures so deleted the oldest figure, which was {figure_numbers[0]}.")
