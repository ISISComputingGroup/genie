...
class WebAggApplication(backend_webagg.WebAggApplication):
    ...
    def initialize(cls, url_prefix='', port=PORT, address=None):
        ...
        try:
            ...
            cls.address = address
            print('before server')
            print('port {}'.format(cls.port))
            cls.server = tornado.httpserver.HTTPServer(app)

            #atexit.register(lambda: cls.server.stop())
            cls.started = True
            ...
            #print('io loop not yet running')
            #cls.ioloop.current().start()
            #print('io loop running')
    ...
    def start(cls):
        ...
        #cls.ioloop = tornado.ioloop.IOLoop.current()
        #IOLoop.make_current()
        #cls.ioloop = IOLoop.current().start()
        cls.ioloop.start()
        print('io loop running')
