...
def _require_runstate(runstates):
    """
        ...
        def _wrapper(self, *args, **kwargs):
            if not kwargs.pop("skip_required_runstates", False):
                ...
                if run_state not in set(runstates):
                    e_string = "{} can only be run in the following runstates: {}".format(func.__name__,
                                                                                          runstates_string)
...
class Dae(object):
    ...
    def __init__(self, api, prefix=""):
        ...
        self.verbose = False
        self.wait_for_completion_callback_dae_settings = True #this is the default value to ensure daesettings are written before returning, only changed for testing
    ...
    def _get_pv_value(self, name, to_string=False, use_numpy=None):
        ...
        """
        return self.api.get_pv_value(name, to_string, use_numpy)
    ...
    @_require_runstate(['RUNNING', 'VETOING', 'WAITING', 'PAUSED'])
    def abort_run(self):
        """
    ...
    @_require_runstate(['RUNNING', 'VETOING', 'WAITING', 'PAUSED', 'ENDING'])
    ...
    @_require_runstate(['RUNNING', 'VETOING', 'WAITING', 'PAUSING'])
    ...
    def _change_dae_settings(self):
        ...
        if changed:
            self._set_pv_value(self._get_dae_pv_name("daesettings_sp"), ET.tostring(root), wait=self.wait_for_completion_callback_dae_settings)
    ...
    def get_spectrum(self, spectrum, period=1, dist=True, use_numpy=None):
        ...
        """
        if dist:
            y_data = self._get_pv_value(self._get_dae_pv_name("getspectrum_y").format(period, spectrum), use_numpy=use_numpy)
            y_size = self._get_pv_value(self._get_dae_pv_name("getspectrum_y_size").format(period, spectrum))
            y_data = y_data[:y_size]
            mode = 'distribution'
            x_size = y_size
        else:
            y_data = self._get_pv_value(self._get_dae_pv_name("getspectrum_yc").format(period, spectrum), use_numpy=use_numpy)
            y_size = self._get_pv_value(self._get_dae_pv_name("getspectrum_yc_size").format(period, spectrum))
            y_data = y_data[:y_size]
            mode = 'non-distribution'
            x_size = y_size + 1
        x_data = self._get_pv_value(self._get_dae_pv_name("getspectrum_x").format(period, spectrum), use_numpy=use_numpy)
    ...
    def _isis_dae_triggered_state_was_reached(self, trigger_pv, state, timeout_per_trigger=20,
        ...
        :return: True if state was reached; False otherwise
        """
        self.api.log_info_msg("Trying to reach state '{}' using trigger pv '{}'".format(state, trigger_pv))
        state_attained = False
        ...
        for _ in range(max_number_of_triggers):
            ...
            self.api.log_error_msg("Failed to get to state '{}' using trigger pv '{}' was in state '{}'".format(
    ...
    @_require_runstate(['SETUP', 'PROCESSING'])
