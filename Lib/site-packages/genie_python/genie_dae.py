...
class Dae(object):
    ...
    def _get_dae_pv_name(self, name, base=False):
        """
    ...
    def begin_run(self, period=None, meas_id=None, meas_type=None, meas_subid=None,
                  sample_id=None, delayed=False, quiet=False, paused=False, prepost=True):
        ...
        if not quiet:
            ...
            try:
                print("The following details will currently be used to determine ownership of the data file")
        ...
        try:
            self.api.logger.log_info_msg(f"BEGIN: Proposal number: {self.get_rb_number()} Team: {self.get_users()}")
        except Exception as e:
        ...
        try:
            ...
            _cancel_monitor_fn = CaChannelWrapper.add_monitor(self._get_dae_pv_name("errormessage"),
                                                              callback_function)
            # actually do begin
            self._set_pv_value(self._get_dae_pv_name("beginrun", base=not prepost), options, wait=True)
        finally:
    ...
    def abort_run(self, prepost=True):
        """
    ...
    def end_run(self, verbose=False, quiet=False, immediate=False, prepost=True):
        """
        End the current run.
        ...
        Args:
            verbose: whether to print verbosely
            quiet: suppress the output to the screen [optional]
            immediate: end immediately, without waiting for a period sequence to complete [optional]
            prepost: run pre and post commands [optional]
        """
        ...
        try:
            self.api.logger.log_info_msg(f"END: Proposal number: {self.get_rb_number()} Team: {self.get_users()}")
        except Exception as e:
    ...
    def pause_run(self, immediate=False, prepost=True):
        """
    ...
    def resume_run(self, prepost=True):
        """
    ...
    def set_period(self, period):
        ...
        if run_state == 'SETUP' or run_state == 'PAUSED':
            ...
            if self.api.get_pv_alarm(self._get_dae_pv_name("period_sp")) == "INVALID":
                raise IOError(f"You are trying to set an invalid period number {period}! "
    ...
    @require_runstate(['PAUSING', 'BEGINNING', 'ABORTING', 'RESUMING', 'RUNNING', 'VETOING', 'WAITING', 'PAUSED'])
    def get_time_since_begin(self, get_timedelta):
        """
    ...
    def change_finish(self):
        ...
        if self.in_transition():
            raise ValueError("Another DAE change operation is currently in progress - values will be inconsistent")
        if self.get_run_state() != "SETUP":
            raise ValueError('Instrument must be in SETUP when changing settings!')
        if self.in_change:
            self.in_change = False
    ...
    def change_sync(self, source):
        ...
        Args:
            source: the source to use ('isis', 'internal', 'smp', 'muon cerenkov', 'muon ms', 'isis (first ts1)', 'isis (ts1 only)')
    ...
    def change_vetos(self, **params):
        ...
        """
        valid_vetoes = [CLEAR_VETO, SMP_VETO, TS2_VETO, HZ50_VETO, EXT0_VETO, EXT1_VETO, EXT2_VETO, EXT3_VETO,
                        FIFO_VETO]
    ...
    def _change_runtime_vetos(self, params):
        ...
        if FIFO_VETO in params:
            if isinstance(params[FIFO_VETO], bool):
                self._set_pv_value(
                    self._get_dae_pv_name("set_veto_" + ("true" if params[FIFO_VETO] else "false")), "FIFO")
    ...
    def configure_hard_periods(self, mode, period_file=None, sequences=None, output_delay=None, period=None, daq=False,
        ...
        Args:
            ...
            Internal periods parameters [optional]:
                ...
                frames: the number of frames to count for the period; if period is not set then applies for all periods
                output: the binary output for the period; if period is not set then applies for all periods
    ...
    def _check_table_file_paths_correct(self, tables_to_check):
    ...
    def _check_tables_not_empty(self):
    ...
    def _change_period_settings(self):
        ...
        if changed:
            ...
            if self.api.get_pv_alarm(self._get_dae_pv_name("periodsettings_sp")) == "INVALID":
                raise IOError("The DAE could not set the number of periods! This may be because you are trying to "
                              "set a number that is too large for the DAE memory. Try a smaller number!")

    def get_spectrum(self, spectrum, period=1, dist=True, use_numpy=None):
        """
        ...
        """
        if dist:
            y_data = self._get_pv_value(self._get_dae_pv_name("getspectrum_y").format(period, spectrum),
                                        use_numpy=use_numpy)
            y_size = self._get_pv_value(self._get_dae_pv_name("getspectrum_y_size").format(period, spectrum))
            y_data = y_data[:y_size]
            mode = 'distribution'
            x_size = y_size
        else:
            y_data = self._get_pv_value(self._get_dae_pv_name("getspectrum_yc").format(period, spectrum),
                                        use_numpy=use_numpy)
            y_size = self._get_pv_value(self._get_dae_pv_name("getspectrum_yc_size").format(period, spectrum))
            y_data = y_data[:y_size]
            mode = 'non-distribution'
            x_size = y_size + 1
        x_data = self._get_pv_value(self._get_dae_pv_name("getspectrum_x").format(period, spectrum),
                                    use_numpy=use_numpy)
    ...
    def get_table_path(self, table_type):
    ...
    def _get_dae_settings_xml(self):

        return ET.fromstring(self._get_pv_value(self._get_dae_pv_name("daesettings")))

    def _wait_for_isis_dae_state(self, state, timeout):
        """
    ...
    def _isis_dae_triggered_state_was_reached(self, trigger_pv, state, timeout_per_trigger=20,
                                              max_number_of_triggers=5):
        """
        Trigger a state and wait for the state to be reached. For example stop the ISIS DAE and wait for it to be
        ...
        :return: True if state was reached; False otherwise
        """
        self.api.logger.log_info_msg("Trying to reach state '{}' using trigger pv '{}'".format(state, trigger_pv))
        state_attained = False
        current_state = "Not set"
        for _ in range(max_number_of_triggers):
            self._set_pv_value(self._prefix_pv_name(trigger_pv), 1)
            state_attained, current_state = self._wait_for_isis_dae_state(state, timeout_per_trigger)
            if state_attained:
                break
        else:
            self.api.logger.log_error_msg("Failed to get to state '{}' using trigger pv '{}' was in state '{}'".format(
                state, trigger_pv, current_state))
        return state_attained

    @contextmanager
    def temporarily_kill_icp(self):
        """
    ...
    def integrate_spectrum(self, spectrum, period=1, t_min=None, t_max=None):
        """
        ...
        if t_min is None:
            ...
            if t_min < time[0]:
                raise ValueError("Argument from_time, {}, is less than lowest bin time, {}.".format(t_min, time[0]))
        ...
        if t_max is None:
            ...
            if t_max > time[-1]:
                raise ValueError("Argument to_time, {}, is greater than highest bin time, {}.".format(t_max, time[-1]))
        ...
        # Calculate missing counts from the lowest bin that needs to be subtracted if this is a partial bin
        ...
        # calculate sum from lowest bin with any counts in to hightest bin that is completely included
        full_count = np.sum(count[first_bin_included: last_complete_bin])

        # run sum of terms, note in the case that the high and low partials are in the same bin this still works
