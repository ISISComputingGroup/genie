...
class Dae(object):
    ...
    def begin_run(
        ...
        if not quiet:
            ...
            try:
                print(
                    "The following details will currently be used to determine ownership of the data file"
                )
    ...
    def abort_run(self, prepost=True):
        ...
        print(
            (
                "** Aborting Run {} at {} (the run will not be saved, call g.recover() to undo this)".format(
    ...
    def change_vetos(self, **params):
        ...
        if len(not_bool) > 0:
            raise Exception(
                "Vetoes must be set to True or False, the following vetoes were incorrect: {}".format(
                    " ".join(not_bool)
                )
            )
    ...
    def _change_period_settings(self):
        ...
        if changed:
            ...
            if self.api.get_pv_alarm(self._get_dae_pv_name("periodsettings_sp")) == "INVALID":
                raise IOError(
                    "The DAE could not set the number of periods! This may be because you are trying to "
    ...
    def _isis_dae_triggered_state_was_reached(
        self, trigger_pv, state, timeout_per_trigger=20, max_number_of_triggers=5
    ):
        """
