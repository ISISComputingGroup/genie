    ...
    def _check_runstate(func):
        ...
        def _wrapper(self, *args, **kwargs):
            ...
            if run_state not in set(runstates):
                e_string = "{} can only be run in the following runstates: {}".format(func.func_name, runstates_string)
...
class Dae(object):
    ...
    @_require_runstate(['RUNNING', 'VETOING', 'WAITING'])
    def pause_run(self):
        """
    ...
    def _change_dae_settings(self):
        ...
        """
        paths_to_check = self._check_tables_not_empty()
       
        if paths_to_check: #if there's nothing in the change cache we don't want to change tables
            
            if self._check_table_file_paths_correct(paths_to_check):
                print("All tables successfully changed.")

            else: # there were some errors, report which tables failed to write.
                errors = " "
                for key, item in paths_to_check.iteritems():
                    if item[1] is False:  # check which item failed and report it
                        errors = "{}{} : {}, ".format(errors, key, item[0])
                raise ValueError("{} table(s) failed to write.\nEnsure you are using raw strings in the filepaths.".format(errors))


    def _check_table_file_paths_correct(self, paths_to_check):
        """ Checks the wiring, detector and spectra tables in 
        the dae settings against those provided in paths_to_check
        
        @param paths_to_check : dictionary containing the change_cache
        ...
        written = True
        for key, item in paths_to_check.iteritems():
            print(item[0], self.get_table_path(key))
            if self.get_table_path(key) == item[0]:
                item[1] = True
                written = written & item[1]  # see if all tables write
            else:
                written = False
        return written
    ...
    def _check_tables_not_empty(self):
        ...
        @returns paths_to_check: a dictionary containing the change_cache
