...
class Dae(object):
    ...
    def begin_run(self, period=None, meas_id=None, meas_type=None, meas_subid=None,
        ...
        self._set_pv_value(self._get_dae_pv_name("beginrun"), options, wait=False)

        def check_state(state):
            if state in ['RUNNING', 'PAUSED', 'VETOING', 'WAITING']:
                return True
            return False

        for attempt in range(15):
            if check_state(self.get_run_state()):
                return True
            if self.get_run_state() != 'BEGINNING':
                break
            time.sleep(1)

        print('\n')
        print(self._get_pv_value(self._get_dae_pv_name("allmessages"), to_string=True), end='\n\n')

        sys.stderr.write("An error has occurred, please close the script and check [DAE -> Run Summary ->"
                         " Recent Log Messages] for more information.")

        # Prevent return if hasn't worked because it might be followed by commands that damage the sample
        while not check_state(self.get_run_state()):
            time.sleep(1)
