...
class ScriptChecker(object):
    ...
    def parse_assignment_target(self, target, suffix=""):
        if suffix:
            suffix = ".{}".format(suffix)

        if isinstance(target, ast.Name):
            return "inst.{}{},".format(target.id, suffix)

        if sys.version_info[0] >= 3 and isinstance(target, ast.Starred):
            return self.parse_assignment_target(target.value, suffix=suffix)

        if isinstance(target, ast.Attribute):
            return self.parse_assignment_target(target.value, suffix=target.attr + suffix)
