...
class ScriptChecker(object):
    ...
    def _check_g_inst_name(self, line, line_no):
        ...
        Return:
            appropriate string of warning if there is an error or else 
            if there is no warning found just return an empty string
        """
    ...
    def check_script(self, name, instrument_name, warnings_as_error=False):
        """
        ...
        """
        errors_output = StringIO.StringIO()
        ...
        Run(["--rcfile={}".format(pylint_path), "--init-hook={}".format(init_hook),
            "--msg-template={C}:{line:3d}: {msg} ({symbol})", "--generated-members={}".format(functions),
             "--score=n", name], reporter=TextReporter(errors_output), exit=False)

        warnings, errors = self.split_warning_errors(errors_output)
            ...
            if filename.endswith(".py") and not filename.startswith("__"):
                ...
                for item in tree.body:
                    ...
                    # getting class and its attributes
                        ...
                        for function_body in item.body:
                            if isinstance(function_body, ast.FunctionDef) and "__" not in function_body.name:
                                attributes += "{class_name}.{funciton_name},".format(class_name=class_name, funciton_name=function_body.name)
                            else:
                                # variables defined inside __init__
                                for variables in function_body.body:
                                    if isinstance(variables, ast.Assign):
                                        attributes += "{class_name}.{variable_name},".format(class_name=class_name, variable_name=variables.targets[0].attr)
