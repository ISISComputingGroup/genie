...
class ScriptChecker(object):
    ...
    def pyright_script_checker(self, script_path, instrument_name):
        ...
        json_write = {
            ...
            "include": [
            ".",
            "C:\\Instrument\\Apps\\Python3\\Lib\\site-packages\\genie_python",
            "C:\\Instrument\\scripts"
            f"C:\\Instrument\\settings\\config\\{instrument_name}\\Python\\inst"
            ],
        ...
        if not os.path.exists(os.path.join(script_dir, config)):
            with open(os.path.join(script_dir, config), "w") as outfile:
                outfile.write(json.dumps(json_write))
        ...
        for diagnostic in json_data["generalDiagnostics"]:
            ...
            errors += [f"On line {start['line'] + 1}:{start['character'] + 1} - {diagnostic['message']}"] if diagnostic['severity'] == "error" else warnings
    ...
    def check_script(self, script_name, instrument_name, warnings_as_error=False):
        ...
        new_warnings, new_errors = self.pyright_script_checker(script_name, instrument_name)
