...
class ScriptChecker(object):
    ...
    def check_script(self, script_name, instrument_name, warnings_as_error=False):
        ...
        Run(["--rcfile={}".format(pylint_path),"--msg-template={C}:{line:3d}: {msg} ({symbol})",
             "--generated-members={}".format(functions), "--score=n", script_name],
            reporter=TextReporter(errors_output), exit=False)
    ...
    def get_inst_attributes(self, instrument_scripts_paths):
        ...
        try:
            ...
            # if the path doesn't exist then there are no inst files so there are no attributes to inst
            return ""
