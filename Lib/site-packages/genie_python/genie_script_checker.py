...
class ScriptChecker(object):
    ...
    def pyright_script_checker(self, script_path):
        ...
        script_dir = f"{os.path.dirname(script_path)}/"
        ...
        if not os.path.exists(f"{script_dir}{config}"):
            cmd += (['copy', f'{os.path.dirname(os.path.realpath(__file__))}\{config}', script_dir, '&&'])

        cmd += ['pyright', '--project', script_dir, '--outputjson', script_path]

        pr_result = subprocess.run(args=cmd, capture_output=True, text=True)
        json_data = json.loads(pr_result.stdout)

        if(json_data['summary']['errorCount'] > 0):
            errors = [json_data["generalDiagnostics"][0]['message']]

        elif json_data['summary']['warningCount'] + json_data['summary']['informationCount'] > 0:
            warnings =  [json_data["generalDiagnostics"][0]['message']]
    ...
    def check_script(self, script_name, instrument_name, warnings_as_error=False):
        ...
        new_warnings, new_errors = self.pyright_script_checker(script_name)
