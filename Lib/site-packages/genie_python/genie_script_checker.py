...
class ScriptChecker(object):
    ...
    def _can_cache_module(self, module_name, module):
        """
        Determines whether a module can be cached or whether the linter should re-examine it's contents each time.
        ...
        # Allow modules defined in the site packages directories to be cached as they are unlikely to change at runtime
        if module.file is not None and any(
                module.file.startswith(site_package_dir) for site_package_dir in site.getsitepackages()):
            return True

        # Other modules are probably user-defined e.g. inst scripts, shared scripts, user scripts. Don't cache these.
        return False

    def _clean_astroid_cache(self):
        """
    ...
    class _TemporaryPyrightConfig():
        ...
        def __init__(self, config_path, instrument_name, additional_include_paths=None):
            ...
            self.json_write = {
                ...
                "extraPaths" : [
                    ...
                    os.path.join("C:\\","Instrument", "settings", "config", instrument_name, "Python"),
                ...
                "reportUndefinedVariable ": False,  # Errors such as these will be caught by pylint before pyright, so no need to report
                "reportMissingImports": False,  # Errors such as these will be caught by pylint before pyright, so no need to report
    ...
    def pyright_script_checker(self, script_path, instrument_name, pyright_additional_include):
        ...
        Args:
            ...
            instrument_name: The instrument name in C:\\Instrument\\Settings\\config\\[instrument_name]\\Python
        Returns:
        ...
        # 1 - Checks to see if pyrightconfig.json is present under the same directory as the selected script
        ...
        with self._TemporaryPyrightConfig(script_dir, instrument_name, pyright_additional_include):
            ...
            cmd = [sys.executable, '-m', 'pyright', '--project', script_dir, '--outputjson', script_path]
            ...
            # for each diagnostic, if severity is error then add message to error array else add to warning array
            for diagnostic in json_data["generalDiagnostics"]:
                start = diagnostic['range']['start']
                if not diagnostic["rule"] == "reportUndefinedVariable": ### REMOVE
                    if diagnostic['severity'] == "error":
                        errors += [f"E: {start['line'] + 1}: {diagnostic['message']} [{diagnostic['rule']}]"]
                    else:
                        warnings += [f"W: {start['line'] + 1}: {diagnostic['message']} [{diagnostic['rule']}]"]

        return warnings, errors
    
    def check_for_tabs(self, lines_list, error_line_numbers, lines_containing_errors):
        ...
        if tab_found:
            print(f'Tab characters found in file, for portability convert tabs to 4 space characters.')
        
    def check_script(self, script_name, instrument_name, warnings_as_error=False, no_pyright=False, no_pylint=False, pyright_additional_include=None):
        """
        ...
        if(no_pylint and no_pyright):
            ...
            try:
                ...
                e_filename = re.search(r'\((.*?),', str(e)).group(1)
                return ["Error while getting attributes of instrument scripts. Please check " + os.path.join(inst_scripts_file_path, e_filename) + ": {}".format(e)]
        ...
        if errors == [] and not no_pyright: # Don't run pryight if pylint goes wrong
            ...
            pyright_warnings, pyright_errors = self.pyright_script_checker(script_name, instrument_name, pyright_additional_include)
        ...
        for line in lines_containing_errors:
            ...
            if g_error_line:
                errors.append(f'An error has been found on line {error_line_numbers[line_numbers_position]} within the file {script_name}')
                errors.append('Please check the "g." statement and ensure that the brackets are not missing.')
    ...
    def parse_assignment_target(self, target, descendants=""):
