...
class ScriptChecker(object):
            ...
            for diagnostic in json_data["generalDiagnostics"]:
                ...
                if not diagnostic["rule"] == "reportUndefinedVariable":  ### REMOVE
                    if diagnostic["severity"] == "error":
                        errors += [
                            f"E: {start['line'] + 1}: {diagnostic['message']} [{diagnostic['rule']}]"
                        ]
                    else:
                        warnings += [
                            f"W: {start['line'] + 1}: {diagnostic['message']} [{diagnostic['rule']}]"
                        ]
    ...
    def check_script(
        ...
        no_pyright=False,
        no_pylint=False,
        pyright_additional_include=None,
    ):
        """
        ...
        if errors == [] and not no_pyright:  # Don't run pryight if pylint goes wrong
            pyright_warnings, pyright_errors = self.pyright_script_checker(
                script_name, instrument_name, pyright_additional_include
            )
        ...
        for line in lines_containing_errors:
            ...
            if g_error_line:
                errors.append(
                    f"An error has been found on line {error_line_numbers[line_numbers_position]} within the file {script_name}"
                )
                errors.append(
