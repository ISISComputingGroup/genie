...
class ScriptChecker(object):
    ...
    class _TemporaryPyrightConfig():
        ...
        def __init__(self, config_path, instrument_name, additional_include_paths=[]):
    ...
    def pyright_script_checker(self, script_path, instrument_name, pyright_additional_include):
        ...
        with self._TemporaryPyrightConfig(script_dir, instrument_name, pyright_additional_include):
            ...
            cmd = [os.path.join(sysconfig.get_paths()["scripts"], "pyright.exe"), '--project', script_dir, '--outputjson', script_path]

            pr_result = subprocess.run(args=cmd, capture_output=True, text=True, encoding="utf-8")
            json_data = json.loads(pr_result.stdout)
            ...
            for diagnostic in json_data["generalDiagnostics"]:
                ...
                message = [f"On line {start['line'] + 1}:{start['character'] + 1} - {diagnostic['message']}"]
                if diagnostic['severity'] == "error": errors += message
    ...
    def check_script(self, script_name, instrument_name, warnings_as_error=False, no_pyright=False, pyright_additional_include=[]):
        """
        ...
        if (len(errors + warnings) == 0) and not no_pyright: # Don't run pryight if pylint goes wrong
