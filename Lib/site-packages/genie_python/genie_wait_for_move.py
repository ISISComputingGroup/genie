...
class WaitForMoveController(object):
    ...
    def _check_timeouts_valid(self, start_timeout, move_timeout):
        if start_timeout is not None and start_timeout <= 0:
            self._api.logger.log_info_msg("Start time out cannot be less than zero - using default value")
            start_timeout = 0
        if move_timeout is not None and move_timeout <= 0:
            self._api.logger.log_info_msg("Move time out cannot be less than zero - using default value")
            move_timeout = None
