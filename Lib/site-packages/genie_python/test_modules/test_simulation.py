...
class TestSimulationSetup(unittest.TestCase):
    ...
    def test_GIVEN_existing_blocks_WHEN_simulate_with_populate_blocks_THEN_blocks_populated(self, block_names):
    ...
    @patch('genie_python.genie._genie_api.get_block_names')
    def test_GIVEN_existing_blocks_WHEN_simulate_with_populate_blocks_and_get_non_existent_block_THEN_exception_thrown(self, block_names):
        block_names.return_value = ["TEST_BLOCK"]
        with genie.sim.Simulate(True):
            self.assertRaises(Exception, genie.cget, "BAD_BLOCK")

    @patch('genie_python.genie._genie_api.get_block_names')
    def test_GIVEN_existing_blocks_WHEN_simulate_with_populate_blocks_and_set_non_existent_block_THEN_exception_thrown(self, block_names):
        block_names.return_value = ["TEST_BLOCK"]
        with genie.sim.Simulate(True):
            self.assertRaises(Exception, genie.cset, "BAD_BLOCK", 10)

    @patch('genie_python.genie._genie_api.get_block_names')
    def test_GIVEN_existing_blocks_WHEN_simulate_without_populate_blocks_and_get_existing_block_THEN_exception_thrown(self, block_names):
        block_names.return_value = ["TEST_BLOCK"]
        with genie.sim.Simulate(True):
    ...
    @patch('genie_python.genie._genie_api.get_block_names')
    def test_WHEN_simulate_without_populate_blocks_and_set_block_THEN_block_is_set(self, block_names):
        with genie.sim.Simulate(False):
    ...
    @patch('genie_python.genie._genie_api.get_block_names')
    def test_WHEN_simulate_with_populate_blocks_and_initial_block_is_set_as_dictionary(self, block_names):
...
class TestSimulationSequence(unittest.TestCase):
    ...
    def test_GIVEN_one_block_WHEN_cset_runcontrol_and_wait__true_THEN_exception(self):
        create_dummy_blocks(["a"])
        with self.assertRaisesRegex(Exception, 'Cannot enable or disable runcontrol at the same time as setting a wait'):
    ...
    def test_GIVEN_multiple_blocks_WHEN_cset_runcontrol_THEN_exception(self):
        ...
        with self.assertRaisesRegex(Exception, 'Runcontrol and wait can only be changed for one block at a time'):
    ...
    def test_GIVEN_period_WHEN_set_period_to_higher_value_THEN_exception(self):
        ...
        # Assert
        with self.assertRaisesRegex(Exception, 'Cannot set period as it is higher than the number of periods'):
