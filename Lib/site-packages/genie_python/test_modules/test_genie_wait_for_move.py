...
class TestGenieWaitForMove(unittest.TestCase):
    ...
    def test_GIVEN_method_flag_error_conditions_WHEN_method_called_THEN_method_doesnt_error(self):
        blocks = []
        self.genie_wait_for_move.flag_error_conditions(blocks)

    def test_GIVEN_method_flag_error_conditions_WHEN_method_called_THEN_method_doesnt_error_A(self):
        blocks = ['BLOCK']
        self.genie_wait_for_move.flag_error_conditions(blocks)

    def test_GIVEN_method_filter_out_missing_blocks_WHEN_method_called_THEN_method_doesnt_error(self):
        blocks = []
    ...
    def test_GIVEN_method_check_timeouts_valid_WHEN_check_timeouts_THEN_returns_valid_values(self, _, start_input, move_input):
        start_timeout = start_input
        move_timeout = move_input
        start_input, move_input = self.genie_wait_for_move._check_timeouts_valid(start_timeout, move_timeout)

        self.assertGreaterEqual(start_input, 0)

    @parameterized.expand([('start_valid_move_none', 10.0, None), ('start_valid_move_invalid', 12.0, -30.0)])
    def test_GIVEN_method_check_timeouts_valid_WHEN_check_timeouts_THEN_returns_valid_values(self, _, start_input, move_input):
        start_timeout = start_input
        move_timeout = move_input
        start_input, move_input = self.genie_wait_for_move._check_timeouts_valid(start_timeout, move_timeout)
