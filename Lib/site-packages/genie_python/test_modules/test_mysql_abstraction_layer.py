...
class TestConnectionLayer(unittest.TestCase):
    ...
    def test_GIVEN_close_error_WHEN_execute_command_THEN_connection_is_closed(self, RaiseIt):
        sql = mysql_abstraction_layer.SQLAbstraction(dbid='exp_data', user='report', password='$report')
        sql._get_connection = Mock(return_value=SQLConnectorStub(RaiseIt))
        ...
        try:
            ...
            assert_that(ExceptionRaised, equal_to(RaiseIt))
    ...
    def test_GIVEN_close_error_WHEN_query_returning_cursor_THEN_connection_is_closed(self, RaiseIt):
        sql = mysql_abstraction_layer.SQLAbstraction(dbid='exp_data', user='report', password='$report')
        sql._get_connection = Mock(return_value=SQLConnectorStub(RaiseIt))
        ExceptionRaised = False
        try:
