...
class TestGenie(unittest.TestCase):
    ...
    def test_WHEN_events_negative_THEN_waitfor_events_raises_error(self):
        with self.assertRaises(ValueError):
            genie.waitfor_events(events=-1)

    @patch('genie_python.genie.__api.get_blocks')
    @patch('genie_python.genie.__api.waitfor.api.dae.get_events')
    def test_WHEN_events_is_0_THEN_waitfor_events_returns(self, events, blocks):
        blocks.return_value = []
        events.return_value = 1
        genie.waitfor_events(events=0)

    def test_GIVEN_events_less_than_2_power_31_WHEN_reported_events_increasing_THEN_waitfor_events_waits_until_reported_events_equals_requested_events(self):
        events = 5000
        self.api = MagicMock()
        self.api.dae.get_events = MagicMock(side_effect=[events - 1, events, events + 1])
        controller = WaitForController(self.api)
        controller.start_waiting(events=events)
        self.assertEqual(self.api.dae.get_events.call_count, 2)

    def test_GIVEN_events_greater_than_2_power_31_WHEN_reported_events_increasing_THEN_waitfor_events_waits_until_reported_events_equals_requested_events(self):
        events = 2 ** 31
        self.api = MagicMock()
        self.api.dae.get_events = MagicMock(side_effect=[events - 1, events, events + 1])
        controller = WaitForController(self.api)
    ...
    def test_GIVEN_events_less_than_2_power_31_WHEN_reported_events_increasing_skips_requested_THEN_waitfor_events_waits_until_next_reported_events_equals_above_requested_events(self):
        events = 5000
        self.api = MagicMock()
        self.api.dae.get_events = MagicMock(side_effect=[events - 2, events - 1, events + 1, events + 2])
        controller = WaitForController(self.api)
        controller.start_waiting(events=events)
        self.assertEqual(self.api.dae.get_events.call_count, 3)

    def test_GIVEN_events_greater_than_2_power_31_WHEN_reported_events_increasing_skips_requested_THEN_waitfor_events_waits_until_next_reported_events_equals_above_requested_events(self):
        events = 2 ** 31
        self.api = MagicMock()
        self.api.dae.get_events = MagicMock(side_effect=[events - 2, events - 1, events + 1, events + 2])
        controller = WaitForController(self.api)
    ...
    @patch('genie_python.genie.__api.get_blocks')
    @patch('genie_python.genie.__api.waitfor.api.dae.get_mevents')
    ...
    def test_WHEN_context_starts_THEN_state_is_connected_and_no_output(self, std_fake_out):
        ...
        self.assertEquals(std_fake_out.getvalue(), '')
