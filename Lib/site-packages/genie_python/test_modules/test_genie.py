...
from genie_python.genie_epics_api import BLOCK_NAMES_MANAGER
...
class TestGenie(unittest.TestCase):
    ...
    def test_WHEN_waitfor_given_maxwait_and_frames_not_reached_THEN_times_out(self, std_fake_out):
        ...
        try:
            ...
            self.assertEquals("Waiting for 5000 frames [timeout=0.1]", output[1])
...
class TestGenieCSet(unittest.TestCase):
    def setUp(self):
        genie.set_instrument(None, import_instrument_init=False)
        genie_python.genie_api_setup._exceptions_raised = True
        self.old_api = getattr(genie, "__api")  # Have to use getattr and setattr because of the name mangling

        mocked_check_alarms = MagicMock(return_value=([], [], []))
        self.mocked_api = MagicMock(check_alarms=mocked_check_alarms)

        setattr(genie, "__api", self.mocked_api)

    def tearDown(self):
        setattr(genie, "__api", self.old_api)
...
class TestGenieCGet(unittest.TestCase):
    def setUp(self):
        genie.set_instrument(None, import_instrument_init=False)
        genie_python.genie_api_setup._exceptions_raised = True
        self.old_api = getattr(genie, "__api")  # Have to use getattr and setattr because of the name mangling
    ...
    def test_GIVEN_non_existent_block_WHEN_cget_called_THEN_raises(self):
        self.mocked_api.block_exists = MagicMock(return_value=False)
        self.assertRaises(Exception, genie.cget, "my_block")
