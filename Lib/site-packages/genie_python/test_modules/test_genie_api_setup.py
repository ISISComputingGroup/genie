...
class TestGenieAutoCompletePyConsole(unittest.TestCase):
    ...
    @parameterized.expand([
        (os.path.join(DRIVE_ROOT,'scri'), "c:/scripts", u"scripts"),
        (os.path.join(DRIVE_ROOT,'scripts'), "c:/scripts", u"scripts"),
        (os.path.join(DRIVE_ROOT,'scripts'), "c:/scripts/play.com", u"scripts/play.com"),
        (os.path.join(DRIVE_ROOT,'scripts','play.'), "c:/scripts/play.py", u"py"),
        (os.path.join(DRIVE_ROOT,'scripts','play.p'), "c:/scripts/play.py", u"py"),
        (os.path.join(DRIVE_ROOT,'scripts','play '), "c:/scripts/play - a.py", u"- a.py"),
        (os.path.join(DRIVE_ROOT,'scripts','play -'), "c:/scripts/play - a.py", u" a.py")
        ])
    @patch('glob.glob')
    def test_GIVEN_line_starts_load_script_with_various_partial_paths_WHEN_auto_complete_THEN_return_contents_of_path_before_dot(self, search_path, filename_from_glob, expected_result, mock_glob):
        # Arrange
    ...
    def test_GIVEN_line_with_g_dot_load_script_with_partial_path_WHEN_auto_complete_THEN_return_contents_of_path(self, mock_glob):
        # Arrange
        search_path = r'c:/scri'
        filenames = [r"scripts"]
        mock_glob.return_value = ["c:/" + f for f in filenames]
        expected_completer = (search_path, [six.text_type(filename) for filename in filenames])

        # Act
        result = self.load_script.pydev_complete(line_buffer=r'g.load_script("{}'.format(search_path))
...
class TestGenieAutoCompletePyDev(unittest.TestCase):
    ...
    @parameterized.expand([
        (os.path.join(DRIVE_ROOT,'scripts','play.'), r"c:/scripts/play.py", "py"),
        (os.path.join(DRIVE_ROOT,'scripts','play.p'), r"c:/scripts/play.py", "py"),
        (os.path.join(DRIVE_ROOT,'scri'), r"c:/scripts", "scripts"),
        (os.path.join(DRIVE_ROOT,'scripts'), r"c:/scripts", "scripts"),
        (os.path.join(DRIVE_ROOT,'scri'), r"c:/scripts","scripts"),
        (os.path.join(DRIVE_ROOT,'scripts','play '), os.path.join(DRIVE_ROOT,"scripts","play - a.py"), "- a.py"),
        (os.path.join(DRIVE_ROOT,'scripts','play -'), os.path.join(DRIVE_ROOT,"scripts","play - a.py"), " a.py"),
        ])
    @patch('glob.glob')
    ...
    @parameterized.expand([
        (u'', SCRIPT_DIR + "scripts.py", u"load_script(\"scripts.py"),
        (u'sc', SCRIPT_DIR + "scripts.py", u"load_script(\"scripts.py"),
        (u'scripts.py', SCRIPT_DIR + "scripts.py", u"py"),
        ])
    @patch('glob.glob')
    def test_GIVEN_line_starts_load_script_with_non_absolute_path_THEN_return_contents_of_scripting_dir(self, search_path, filename_from_glob, expected_result, mock_glob):
        # Arrange
        mock_glob.return_value = [filename_from_glob]
        expected_completer = ("c:/scripts/" + search_path, [expected_result])
    ...
    def test_GIVEN_line_starts_load_script_with_nothing_else_THEN_return_contents_of_scripting_dir_with_load_script(self, mock_glob):
        ...
        expected_completer = ("c:/scripts/", [expected_result])
...
class TestLogAndExceptionDecorator(unittest.TestCase):
    @patch("genie_python.genie_api_setup.__api.log_command")
    def test_GIVEN_decorated_method_that_raises_and_squshed_exceptions_WHEN_called_THEN_exception_squashed(self, logger):
    ...
    @patch("genie_python.genie_api_setup.__api.log_command")
    def test_GIVEN_decorated_method_that_raises_and_no_squashed_exception_WHEN_called_THEN_exception_raised(self, logger):
        genie_python.genie_api_setup._exceptions_raised = True
    ...
    def test_GIVEN_decorated_method_WHEN_call_with_no_args_THEN_logs_name(self, logger):
        ...
        test_func()
        logger.assert_called_with("test_func", {'kwargs': {}, 'args': {}})
    ...
    def test_GIVEN_decorated_method_WHEN_call_with_named_args_THEN_logs_name_and_args(self, logger):
        ...
        test_func(1, 2, 3)
        logger.assert_called_with("test_func", {'kwargs': {}, 'args': {"arg_one": 1, "arg_two": 2, "arg_three": 3}})
    ...
    def test_GIVEN_decorated_method_WHEN_call_with_kwargs_THEN_logs_name_and_kwargs(self, logger):
        ...
        logger.assert_called_with("test_func", {'kwargs': {"kwarg_one": 1, "kwarg_two": 2}, 'args': {}})

    @patch("genie_python.genie_api_setup.__api.log_command")
    def test_GIVEN_decorated_method_WHEN_call_with_mixed_args_and_kwargs_THEN_logs_name_args_and_kwargs(self, logger):
        @log_command_and_handle_exception
        def test_func(arg_one, **kwargs):
            pass

        test_func(1, kwarg_one=1, kwarg_two=2)
        logger.assert_called_with("test_func", {'kwargs': {"kwarg_one": 1, "kwarg_two": 2}, 'args': {"arg_one": 1}})

    @patch("genie_python.genie_api_setup.__api.log_command")
    def test_GIVEN_decorated_method_WHEN_call_with_mixed_named_args_and_args_THEN_logs_args_without_name(self, logger):
        @log_command_and_handle_exception
        def test_func(arg_one, *args):
            pass

        test_func(1, 2, 3)
        logger.assert_called_with("test_func", {'kwargs': {}, 'args': (1, 2, 3)})
