...
from genie_python.genie_simulate_impl import Dae as SimDae, ChangeCache as SimChangeCache
import six
...
</Cluster>""".format(wiring_path=WIRING_PATH, detector_path=DETECTOR_PATH, spectra_path=SPECTRA_PATH)
...
class TestGenieDAE(unittest.TestCase):
    ...
    def test_GIVEN_not_in_change_WHEN_change_finish_called_THEN_value_error_with_correct_message_thrown(self):
        self.dae.in_change = False

        self.assertRaisesRegexp(ValueError, "Change has already finished", self.dae.change_finish)

    def test_GIVEN_in_transition_WHEN_change_finish_called_THEN_value_error_with_correct_message_thrown(self):
        self.dae.in_change = True
        self.dae.in_transition = MagicMock(return_value=True)

        self.assertRaisesRegexp(ValueError, "Another DAE change operation is currently in progress - values will be "
                                            "inconsistent", self.dae.change_finish)

    def test_GIVEN_instrument_not_in_setup_WHEN_change_finish_called_THEN_value_error_with_correct_message_thrown(self):
        self.dae.in_change = True
        self.dae.get_run_state = MagicMock(return_value="RUNNING")

        self.assertRaisesRegexp(ValueError, "Instrument must be in SETUP when changing settings!",
                                self.dae.change_finish)

    def test_GIVEN_instrument_in_setup_and_in_change_and_not_in_transition_WHEN_change_finish_called_THEN_no_exception_thrown_and_not_in_change(self):
        self.dae.in_change = True
    ...
    def test_GIVEN_number_of_periods_pv_in_invalid_alarm_WHEN_change_period_settings_called_THEN_exception_thrown(self):
    ...
    def test_GIVEN_number_of_periods_pv_no_alarm_WHEN_change_finish_called_THEN_no_exception_thrown(self):
    ...
    def test_WHEN_get_spectrum_dist_true_THEN_default_returns_regular_counts(self):
        ...
        self.assertEqual(spectrum["signal"], Y_RETURN[:Y_NORD_RETURN], "Should return value of get_spectrum_y")
        self.assertEqual(len(spectrum["signal"]), len(spectrum["time"]), "Should be the same length")
    ...
    def test_WHEN_get_spectrum_dist_false_THEN_default_returns_pure_counts(self):
        ...
        self.assertEqual(spectrum["signal"], YC_RETURN[:YC_NORD_RETURN], "Should return value of get_spectrum_yc")
        self.assertEqual(len(spectrum["signal"])+1, len(spectrum["time"]), "x_size should be one larger than y_size")
    ...
    def test_WHEN_get_spectrum_integrate_for_count_partial_first_bin_top_limit_not_on_boundry_THEN_get_part_of_count(self):
    ...
    def test_WHEN_get_spectrum_integrate_for_count_partial_first_bin_lower_limit_no_on_boundryTHEN_get_part_of_count(self):
    ...
    def test_WHEN_get_spectrum_integrate_for_count_partial_of_two_different_bins_none_apart_THEN_get_part_of_count(self):

        self.api.get_pv_value = MagicMock(side_effect=self.get_y_or_yc_pv_value)

        result = self.dae.integrate_spectrum(1, 1, X_RETURN[1] + WIDTHS[1] * 0.25, X_RETURN[2] + WIDTHS[2] * 0.4)
    ...
    def test_WHEN_get_spectrum_integrate_for_count_partial_of_two_different_bins_one_apart_THEN_get_part_of_count(self):

        self.api.get_pv_value = MagicMock(side_effect=self.get_y_or_yc_pv_value)

        result = self.dae.integrate_spectrum(1, 1, X_RETURN[1] + WIDTHS[1] * 0.25, X_RETURN[3] + WIDTHS[3] * 0.4)

        expected_from_low_bin = YC_RETURN[1] * 0.75
        expected_from_middle_bin = YC_RETURN[2]
        expected_from_high_bin = YC_RETURN[3] * 0.4
        assert_that(result, is_(close_to(expected_from_high_bin + expected_from_middle_bin + expected_from_low_bin, 1e-6)))

    def test_WHEN_get_spectrum_integrate_for_count_partial_of_one_bins_THEN_get_part_of_count(self):

        self.api.get_pv_value = MagicMock(side_effect=self.get_y_or_yc_pv_value)

        result = self.dae.integrate_spectrum(1, 1, X_RETURN[1] + WIDTHS[1] * 0.25, X_RETURN[1] + WIDTHS[1] * 0.4)
    ...
    def test_WHEN_get_spectrum_integrate_and_low_limit_is_below_lowest_bin_THEN_error(self):
        ...
        assert_that(calling(self.dae.integrate_spectrum).with_args(1, 1, X_RETURN[0] - 0.01, X_RETURN[1]),
                    raises(ValueError))
    ...
    def test_WHEN_get_spectrum_integrate_and_upper_limit_is_above_highest_bin_THEN_error(self):
        ...
        assert_that(calling(self.dae.integrate_spectrum).with_args(1, 1, X_RETURN[0], X_RETURN[YC_NORD_RETURN] + 0.1),
                    raises(ValueError))
    ...
    def test_WHEN_get_spectrum_integrate_and_upper_limit_is_below_lower_limit_THEN_error(self):
        ...
        assert_that(calling(self.dae.integrate_spectrum).with_args(1, 1, 10, 9),
                    raises(ValueError))
    ...
    def test_WHEN_users_set_with_multiple_users_THEN_new_users_sent_to_db_server(self):
        ...
        self.assertEqual("ED:USERNAME:SP", set_pv_arguments[0])
        self.assertEqual('[{"name": "Smith"}, {"name": "Jones"}]', dehex_and_decompress(set_pv_arguments[1]))
        self.assertEqual(True, set_pv_arguments[2])
...
class TestGenieAndSimulateDAEParity(unittest.TestCase):
    ...
    @patch("genie_python.genie_cachannel_wrapper.CaChannelWrapper.add_monitor")
    def test_GIVEN_in_setup_state_WHEN_begin_run_called_THEN_no_exception_thrown(self, mock_monitor):
        mock_monitor.return_value = None
