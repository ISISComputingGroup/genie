...
import json
import unittest
from genie_python.utilities import compress_and_hex, dehex_and_decompress, waveform_to_string, get_correct_path, crc8, \
    EnvironmentDetails, get_json_pv_value, PVReadException, convert_string_to_ascii, remove_field_from_pv
...
class TestUtilitiesSequence(unittest.TestCase):
    ...
    def create_waveform_from_list(input_list):
        ...
        if six.PY2:
            return "".join([unichr(i) for i in input_list])
        else:
            return "".join([chr(i) for i in input_list])

    def test_GIVEN_short_list_of_strings_WHEN_waveform_converted_to_string_THEN_result_contains_a_string_of_strings(self):
        # Arrange
...
class TestCRC8Util(unittest.TestCase):
    def calc_and_test(self, expected, inst):
        result = crc8(inst)
        self.assertEqual(result, expected, 'CRC value was "{0}" expected "{1}"'.format(result, expected))
...
class TestGetJsonPVValue(unittest.TestCase):
    def test_GIVEN_invalid_json_WHEN_get_pv_THEN_list_raise(self):
        ...
        self.assertRaisesRegex(PVReadException, "Can not unmarshal.*", get_json_pv_value, "name", api)
    ...
    def test_GIVEN_invalid_compressed_string_WHEN_get_pv_THEN_raise(self):
        ...
        self.assertRaisesRegex(PVReadException, "Can not decompress.*", get_json_pv_value, "name", api)
