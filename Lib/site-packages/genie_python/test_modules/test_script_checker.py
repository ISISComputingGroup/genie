...
class TestScriptChecker(unittest.TestCase):
    ...
    def test_GIVEN_g_assignment_with_2_symbols_before_number_WHEN_check_THEN_warning_message(self):
        ...
        temp_script_file.close()
        os.unlink(temp_script_file.name)
        self.assertEqual(result, ['"g" is unbound', "W:  3: 'g' assignment in line 3"]) # Updated to account for pyright
    ...
    def test_GIVEN_new_directory_WHEN_pyright_script_checker_called_THEN_new_pyright_json_created(self):

        script_lines = ["a = 1"]
        
        temp_dir = tempfile.mkdtemp()
        temp_script_file = write_to_file(script_lines, suffix=".py", dir=temp_dir)

        self.checker.pyright_script_checker(temp_script_file.name, self.instrument)
    ...
    def test_GIVEN_that_pylint_fails_THEN_pyright_is_not_ran(self):
        ...
        errors = self.checker.check_script(temp_script_file.name, self.machine, warnings_as_error=True)
        self.assertTrue(len(errors) == 1) # Will be count of 2 if pyright still runs
        os.unlink(temp_script_file.name)
