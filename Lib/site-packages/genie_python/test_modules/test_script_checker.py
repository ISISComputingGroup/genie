...
class TestScriptChecker(unittest.TestCase):
    ...
    def test_GIVEN_python_file_WHEN_call_THEN_give_lists_of_attributes(self):
        ...
        temp_file = tempfile.NamedTemporaryFile(suffix = ".py", delete=False, dir=dir_path)
    ...
    def test_GIVEN_valid_python_expr_WHEN_call_check_THEN_no_error(self):
        ...
        result = self.checker.check_script(temp_script_file.name, self.machine)
        os.unlink(temp_script_file.name)
        self.assertEquals(result, ['E:  1: invalid syntax (<unknown>, line 1) (syntax-error)'])

    def test_GIVEN_invalid_python_expr_WHEN_call_check_THEN_no_error(self):
        script_lines = ["my_expr = {}"]
        temp_script_file = write_to_file(script_lines)
        result = self.checker.check_script(temp_script_file.name, self.machine)
        os.unlink(temp_script_file.name)
    ...
    def test_GIVEN_invalid_python_class_WHEN_call_check_THEN_no_error(self):
        script_lines = ["class MyClass():"]
        temp_script_file = write_to_file(script_lines)
        result = self.checker.check_script(temp_script_file.name, self.machine)
        os.unlink(temp_script_file.name)
        self.assertEquals(result, ['E:  1: unexpected EOF while parsing (<unknown>, line 1) (syntax-error)'])
