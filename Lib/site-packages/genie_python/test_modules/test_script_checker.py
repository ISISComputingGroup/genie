...
class TestScriptChecker(unittest.TestCase):
    ...
    class _CreateTempScriptAndReturnErrors():
        ...
        def __init__(self, script_checker, script, warnings_as_error=True, no_pyright=False, no_pylint=False, dir=""):
            self.script = script
            self.dir=dir
        ...
        def __enter__(self):
            ...
            return self.script_checker.checker.check_script(self.temp_script_file.name, self.script_checker.machine, warnings_as_error=self.warnings_as_error, no_pyright=self.no_pyright, no_pylint=self.no_pylint)
    ...
    def test_GIVEN_end_without_brackets_WHEN_check_THEN_error_message(self):
        ...
        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertEqual(errors, ["W:  4: Statement seems to have no effect (pointless-statement)"])
    ...
    def test_GIVEN_end_without_brackets_at_start_of_line_WHEN_check_THEN_error_message(self):
        ...
        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertEqual(errors, ["W:  3: Statement seems to have no effect (pointless-statement)"])
    ...
    def test_GIVEN_end_without_brackets_on_line_with_fn_with_brackets_WHEN_check_THEN_error_message(self):
        ...
        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertEqual(errors, ["W:  2: Statement seems to have no effect (pointless-statement)"])
    ...
    def test_GIVEN_function_with_g_WHEN_check_THEN_warn_user(self):
        ...
        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertEqual(errors, ["E:  3: Module 'genie_python.genie' has no 'test_function' member (no-member)"])
    ...
    def test_GIVEN_2_g_assignments_WHEN_check_THEN_warning_message(self):
        ...
        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertEqual(errors, ["W:  3: 'g' assignment in line 3", "W:  4: 'g' assignment in line 4"])
    ...
    def test_GIVEN_g_non_existing_command_WHEN_call_THEN_error_message(self):
        ...
        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertEqual(errors, ["E:  3: Module 'genie_python.genie' has no "
                                   "'aitfor_time' member; maybe 'waitfor_time'? (no-member)"])        

    def test_GIVEN_class_definition_WHEN_get_inst_attributes_THEN_class_name_and_attributes_defined(self):
        script_lines = ["class TestClass(Object):\n",
                        "    def __init__(self):\n",
                        "        self.a = 10"]
    ...
    def test_GIVEN_unparenthesized_tuple_assignment_WHEN_get_inst_attributes_THEN_all_vars_defined(self):
        script_lines = ["a,b = 1,2"]

        self.assertSymbolsDefined(script_lines, "inst.a,inst.b")

    def test_GIVEN_parenthesized_tuple_assignment_WHEN_get_inst_attributes_THEN_all_vars_defined(self):
        script_lines = ["(a,b) = (1,2)"]
    ...
    def test_GIVEN_index_subscript_assignment_WHEN_get_inst_attributes_THEN_no_symbols_defined(self):
        script_lines = ["list[0] = 1"]

        self.assertSymbolsDefined(script_lines, "")

    def test_GIVEN_list_definition_and_index_subscript_assignment_WHEN_get_inst_attributes_THEN_only_list_defined(self):
        script_lines = ["list = []\n",
                        "list[0] = 1"]
    ...
    def test_GIVEN_dict_definition_and_subscript_assignment_WHEN_get_inst_attributes_THEN_only_dict_defined(self):
        script_lines = ["dict = {}\n",
                        "dict['a'] = 1"]

        self.assertSymbolsDefined(script_lines, "inst.dict")

    def test_GIVEN_slice_subscript_assignment_WHEN_get_inst_attributes_THEN_no_symbols_defined(self):
        script_lines = ["list[0:2] = [8,9]"]

        self.assertSymbolsDefined(script_lines, "")

    def test_GIVEN_list_definition_and_slice_subscript_assignment_WHEN_get_inst_attributes_THEN_only_list_defined(self):
        script_lines = ["list = []\n",
                        "list[0:2] = [8,9]"]
    ...
    def test_GIVEN_nested_attribute_assignment_WHEN_get_inst_attributes_THEN_attribute_defined(self):
    ...
    @unittest.skipIf(sys.version_info[0] < 3, "Starred assignments are not supported in Python 2")
    def test_GIVEN_starred_complex_assignment_WHEN_get_inst_attributes_THEN_all_symbols_defined(self):
    ...
    def test_GIVEN_instrument_script_THEN_it_should_not_be_cached(self):
        name = "my_inst_script"
        mod = astroid.Module(name, os.path.join(
            "C:\\", "Instrument", "Settings", "config", "UNITTEST", "Python", "inst", "my_inst_script.py"))
        self.assertFalse(self.checker._can_cache_module(name, mod))
    ...
    def test_GIVEN_invalid_var_type_THEN_pyright_throws_exception(self):
        ...
        expected = "E: 2: Expression of type"

        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertTrue(errors[0].startswith(expected))

    def test_GIVEN_new_directory_WHEN_pyright_script_checker_called_THEN_pyright_json_created_then_destroyed_after_use(self):
    ...
    def test_GIVEN_invalid_genie_script_WHEN_pyright_script_checker_called_THEN_pyright_throws_exception(self):
    ...
    def test_GIVEN_that_pylint_fails_THEN_pyright_is_not_ran(self):
        ...
        with self._CreateTempScriptAndReturnErrors(self, script_lines) as errors:
            self.assertTrue(errors[0].startswith(expected) and len(errors) == 1) # Will be count of 2 if pyright still runs
    ...
    def test_GIVEN_invalid_genie_script_WHEN_pyright_script_checker_called_with_no_pyiright_as_true_THEN_no_error(self):
    ...
    def test_GIVEN_trying_to_access_member_of_optional_type_var_WHEN_pyright_script_checker_called_THEN_no_error(self):

        script_lines = ["from typing import Optional\n",
    ...
    def test_GIVEN_trying_to_index_var_of_optional_type_WHEN_pyright_script_checker_called_THEN_no_error(self):

        script_lines = ["from typing import Optional, List\n",
    ...
    def test_GIVEN_trying_to_call_var_of_optional_type_WHEN_pyright_script_checker_called_THEN_no_error(self):

        script_lines = ["from typing import Optional, Callable\n",
    ...
    def test_GIVEN_trying_to_iterate_over_var_of_optional_type_WHEN_pyright_script_checker_called_THEN_no_error(self):
    ...
    def test_GIVEN_trying_to_define_function_with_none_type_args_type_WHEN_pyright_script_checker_called_THEN_no_error(self):
    ...
    def test_GIVEN_trying_to_use_optional_operand__WHEN_pyright_script_checker_called_THEN_no_error(self):
    ...
    def test_GIVEN_trying_to_use_undefined_variable_WHEN_pyright_script_checker_called_THEN_no_error(self):
