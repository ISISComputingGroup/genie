...
class TestScriptChecker(unittest.TestCase):
    ...
    def assertSymbolsDefined(self, script_lines, expected_symbols):
        ...
        self.assertEquals(result, expected_symbols)
    ...
    def test_GIVEN_end_without_brackets_on_line_with_fn_with_brackets_WHEN_check_THEN_error_message(self):
        ...
        self.assertEquals(result, ["W:  2: Statement seems to have no effect (pointless-statement)"])
    ...
    def test_GIVEN_g_assignment_with_2_symbols_before_number_WHEN_check_THEN_warning_message(self):
        ...
        temp_script_file.close()
        os.unlink(temp_script_file.name)
        self.assertEquals(result, ["E:  3: Undefined variable 'g' (undefined-variable)",
    ...
    def test_GIVEN_g_non_existing_command_WHEN_call_THEN_error_message(self):
        ...
        os.unlink(temp_script_file.name)
        self.assertEquals(result, ["E:  3: Module 'genie_python.genie' has no "
                                   "'aitfor_time' member; maybe 'waitfor_time'? (no-member)"])

    def test_GIVEN_python_file_WHEN_call_THEN_give_lists_of_attributes(self):
        script_lines = ["def test():\n",
                        "    b = 10\n",
        ...
        result = self.checker.get_inst_attributes(dir_path)
        shutil.rmtree(dir_path)
        self.assertEquals(result, "inst.test,inst.ScriptChecker,inst.ScriptChecker.test,inst.GLOBAL")
