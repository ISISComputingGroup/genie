...
from genie_python.genie_epics_api import API, BLOCK_NAMES_MANAGER
...
class TestEpicsApiSequence(unittest.TestCase):
    ...
    def mock_get_pv_value(pv_name, to_string):
        ...
        Returns:
            A string saying "INVALID" if the pv name argument is ALARM_PV.SEVR, or an exception if not.
        """
...
class TestEpicsApiSetInstrumentName(unittest.TestCase):
    ...
    def test_WHEN_machine_identifier_begins_ndh_THEN_instrument_is_name_without_ndh(self):
        # Act
        expected = "NAME"
        instrument, machine, pv_prefix = self.api._get_machine_details_from_identifier("NDH"+expected)
...
class TestPvMethods(unittest.TestCase):
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_value_of_local_pv_with_local_prefix_on_start_THEN_pv_is_returned_and_pv_asked_for_has_only_one_prefix(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
        pv_wrapper_mock.get_pv_value.return_value = expected_value
        pv_name = "PV"
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_value_but_pv_does_not_exist_THEN_exception(self, pv_wrapper_mock):
        pv_wrapper_mock.pv_exists.return_value = False

        assert_that(calling(self.api.get_pv_value).with_args("PV"), raises(UnableToConnectToPVException))

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_value_but_wrapper_exception_THEN_exception_thrown(self, pv_wrapper_mock):
        ...
        assert_that(pv_wrapper_mock.get_pv_value.call_count, is_(3), "get pv call count, one for each attempt")

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_value_wrapper_exception_on_first_trial_and_ok_on_second_THEN_value_returned(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
        ...
        assert_that(result, is_(expected_value))
        pv_wrapper_mock.get_pv_value.assert_called_with(expected_pv_name, False, use_numpy=None)
        assert_that(pv_wrapper_mock.get_pv_value.call_count, is_(2), "get pv call count, once for raise once for ok")

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_value_raises_exception_except_on_last_trial_THEN_value_returned(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
        pv_wrapper_mock.get_pv_value.side_effect = [ValueError(), ValueError(), ValueError(), expected_value]
        expected_pv_name = "PV"
        ...
        assert_that(result, is_(expected_value))
        pv_wrapper_mock.get_pv_value.assert_called_with(expected_pv_name, False, use_numpy=None)
        assert_that(pv_wrapper_mock.get_pv_value.call_count, is_(4), "get pv call count, once for raise once for ok")
    ...
    def test_GIVEN_mock_pv_WHEN_set_local_pv_value_THEN_pv_is_set(self, pv_wrapper_mock):
        ...
        pv_wrapper_mock.set_pv_value.assert_called_with(expected_pv_name, expected_value, wait=False)

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_value_with_inst_prefix_THEN_pv_is_set_pv_does_not_have_extra_prefix(self, pv_wrapper_mock):
        expected_value = 10
        pv_name = "PV"
        expected_pv_name = "{}{}".format(self.instrument_prefix, pv_name)
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_value_but_wrapper_exceptions_THEN_exception_thrown(self, pv_wrapper_mock):
        pv_wrapper_mock.set_pv_value.side_effect = ValueError()
        expected_value = 10
        pv_name = "PV"
        expected_pv_name = "{}{}".format(self.instrument_prefix, pv_name)

        assert_that(calling(self.api.set_pv_value).with_args(expected_pv_name, expected_value, is_local=True),
                    raises(ValueError))
        assert_that(pv_wrapper_mock.set_pv_value.call_count, is_(3), "set pv call count, once for raise once for ok")

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_value_but_wrapper_exception_on_first_trial_and_ok_on_second_THEN_pv_value_set(self, pv_wrapper_mock):
        expected_value = 10
        pv_name = "PV"
        expected_pv_name = "{}{}".format(self.instrument_prefix, pv_name)
        ...
        pv_wrapper_mock.set_pv_value.assert_called_with(expected_pv_name, expected_value, wait=False)
        assert_that(pv_wrapper_mock.set_pv_value.call_count, is_(2), "set pv call count, once for raise once for ok")

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_value_raises_exception_except_on_last_trial_THEN_pv_value_set_called_attempt_times(self, pv_wrapper_mock):
        expected_value = 10
        pv_name = "PV"
        expected_pv_name = "{}{}".format(self.instrument_prefix, pv_name)

        pv_wrapper_mock.set_pv_value.side_effect = [ValueError(), ValueError(), ValueError(), expected_value]

        self.api.set_pv_value(expected_pv_name, expected_value, is_local=True, attempts=4)

        pv_wrapper_mock.set_pv_value.assert_called_with(expected_pv_name, expected_value, wait=False)
        assert_that(pv_wrapper_mock.set_pv_value.call_count, is_(4), "set pv call count, once for raise once for ok")

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_block_pointing_at_field_WHEN_get_block_units_THEN_units_field_is_called(self, pv_wrapper_mock):
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_block_already_pointing_at_unit_field_WHEN_get_block_units_THEN_units_field_is_called(self, pv_wrapper_mock):
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_block_not_pointing_at_field_WHEN_get_block_units_THEN_units_field_is_called(self, pv_wrapper_mock):
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_block_not_found_WHEN_get_block_units_THEN_exception_is_raised(self, pv_wrapper_mock):
        # Mock get_pv_from_block to return something without .SOMETHING at the end
        self.api.get_pv_from_block = MagicMock(return_value="PVNAME")
        ...
        # Assert get_pv_value is called with the .EGU
        assert_that(calling(self.api.get_block_units).with_args("BLOCK_NAME"),
                    raises(Exception))

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_pv_not_found_WHEN_get_block_units_THEN_exception_is_raised(self, pv_wrapper_mock):
        # Mock get_pv_from_block to return something without .SOMETHING at the end
        self.api.get_pv_from_block = MagicMock(return_value="PVNAME")
        ...
        # Assert get_pv_value is called with the .EGU
        assert_that(calling(self.api.get_block_units).with_args("BLOCK_NAME"), raises(UnableToConnectToPVException))
...
class TestSetBlockMethod(unittest.TestCase):
    ...
    @patch("genie_python.genie_epics_api.API.blockserver")
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_WHEN_set_block_value_called_with_wait_THEN_setpoint_set_and_wait_for_called(self, pv_wrapper, blockserver):
        self.api.waitfor = MagicMock()

        self.api.set_block_value("TEST_BLOCK", 10, wait=True)
        pv_wrapper.set_pv_value.assert_called_with(self.api.get_pv_from_block("TEST_BLOCK") + ":SP", 10, wait=False)
        self.api.waitfor.start_waiting.assert_called()

    @patch("genie_python.genie_epics_api.API.blockserver")
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_WHEN_set_block_value_called_with_wait_and_high_low_THEN_wait_for_called_with_high_low(self, pv_wrapper, blockserver):
        self.api.waitfor = MagicMock()
        block_name, set_point = "TEST_BLOCK", 10
        lowlimit, highlimit = 8, 12

        self.api.set_block_value(block_name, set_point, wait=True, lowlimit=lowlimit, highlimit=highlimit)
        self.api.waitfor.start_waiting.assert_called_with(block_name, set_point, lowlimit, highlimit)

    @patch("genie_python.genie_epics_api.API.blockserver")
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_WHEN_set_block_value_called_with_wait_and_high_low_incorrect_order_THEN_wait_for_called_with_high_low_correct_order(self, pv_wrapper, blockserver):
        self.api.waitfor = MagicMock()
        block_name, set_point = "TEST_BLOCK", 10
        lowlimit, highlimit = 12, 8

        self.api.set_block_value(block_name, set_point, wait=True, lowlimit=lowlimit, highlimit=highlimit)
        self.api.waitfor.start_waiting.assert_called_with(block_name, set_point, highlimit, lowlimit)

    @patch("genie_python.genie_epics_api.API.blockserver")
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_WHEN_set_block_value_called_with_wait_and_high_low_THEN_wait_for_called_with_high_low_correct_order(self, pv_wrapper, blockserver):
        self.api.waitfor = MagicMock()
        block_name, set_point = "TEST_BLOCK", 10
        lowlimit, highlimit = 12, 8
    ...
    def test_WHEN_set_block_value_called_with_wait_and_various_odd_high_lows_THEN_message_printed(self, _, low, high, val, should_warn, mock_print, pv_wrapper, blockserver):
        self.api.waitfor = MagicMock()
        block_name, set_point = "TEST_BLOCK", 10

        self.api.set_block_value(block_name, val, wait=True, lowlimit=low, highlimit=high)
        if should_warn:
            mock_print.assert_called_with("Warning the range {} to {} does not cover setpoint of {}, may wait forever".format(low, high, val))
        else:
    ...
    def test_GIVEN_non_existent_block_but_in_blockserver_names_WHEN_cget_called_THEN_returns_disconnected(self):
        test_block = "my_block"
        self.api.block_exists = MagicMock(return_value=False)
    ...
    def test_GIVEN_disconnected_block_WHEN_cget_called_THEN_still_tries_to_get_runcontrol_and_alarms(self):
        test_block = "my_block"
    ...
    def test_GIVEN_disconnected_block_WHEN_cget_called_with_fail_fast_THEN_still_tries_to_get_runcontrol_but_not_alarms(self):
        test_block = "my_block"
        expected_run_control = "YES", "0", "10"
        self.api.block_exists = MagicMock(return_value=False)
        self.api.get_block_names = MagicMock(return_value=[test_block])
...
class TestLoggingFunctions(unittest.TestCase):
    ...
    def test_given_a_date_time_and_a_log_dir_when_getting_log_file_name_then_returns_correct_file_name(self):
        date_time = "time_string"
        log_dir = "\\example\\"
        expected = f"{log_dir}genie-{date_time}.log"
        actual = API._get_log_file_name(log_dir, date_time)
