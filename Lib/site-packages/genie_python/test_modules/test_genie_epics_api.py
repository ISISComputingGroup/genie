...
class TestPvMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            if e.message != "No module named init_test":
                raise

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_local_pv_THEN_pv_is_returned(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
        pv_wrapper_mock.get_pv_value.return_value = expected_value
        pv_name = "PV"
        expected_pvname = "{}{}".format(self.instrument_prefix, pv_name)

        result = self.api.get_pv_value(pv_name, is_local=True)

        assert_that(result, is_(expected_value))
        pv_wrapper_mock.get_pv_value.assert_called_with(expected_pvname, False)

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_local_pv_with_local_prefix_on_start_THEN_pv_is_returned_and_pv_asked_for_has_only_one_prefix(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
        pv_wrapper_mock.get_pv_value.return_value = expected_value
        pv_name = "PV"
        ...
        result = self.api.get_pv_value(expected_pvname, is_local=True)

        assert_that(result, is_(expected_value))
        pv_wrapper_mock.get_pv_value.assert_called_with(expected_pvname, False)

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_but_pv_does_not_exist_THEN_exception(self, pv_wrapper_mock):
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_THEN_pv_is_returned(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
        pv_wrapper_mock.get_pv_value.return_value = expected_value
        expected_pvname = "PV"

        result = self.api.get_pv_value(expected_pvname)

        assert_that(result, is_(expected_value))
        pv_wrapper_mock.get_pv_value.assert_called_with(expected_pvname, False)

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_but_exception_THEN_exception_thrown(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.side_effect = ValueError()

        assert_that(calling(self.api.get_pv_value).with_args("PV"), raises(ValueError))

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_exception_on_first_trial_and_ok_on_second_THEN_value_returned(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_get_pv_raises_exception_expect_on_last_trial_THEN_value_returned(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.pv_exists.return_value = True
        pv_wrapper_mock.get_pv_value.side_effect = [ValueError(), ValueError(), ValueError(), expected_value]
        expected_pvname = "PV"

        result = self.api.get_pv_value(expected_pvname, attempts=4)

        assert_that(result, is_(expected_value))
        pv_wrapper_mock.get_pv_value.assert_called_with(expected_pvname, False)

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_THEN_pv_is_set(self, pv_wrapper_mock):
        expected_value = 10
        pv_name = "PV"
        expected_pvname = "{}{}".format(self.instrument_prefix, pv_name)
        ...
        pv_wrapper_mock.set_pv_value.assert_called_with(expected_pvname, expected_value, wait=False)

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_with_inst_prefix_THEN_pv_is_set_pv_does_not_have_extra_prefix(self, pv_wrapper_mock):
        expected_value = 10
        pv_name = "PV"
        expected_pvname = "{}{}".format(self.instrument_prefix, pv_name)

        self.api.set_pv_value(expected_pvname, expected_value, is_local=True)

        pv_wrapper_mock.set_pv_value.assert_called_with(expected_pvname, expected_value, wait=False)

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_and_set_raises_THEN_set_raises(self, pv_wrapper_mock):
        pv_wrapper_mock.set_pv_value.side_effect = ValueError()
        expected_value = 10
        pv_name = "PV"
        expected_pvname = "{}{}".format(self.instrument_prefix, pv_name)

        assert_that(calling(self.api.set_pv_value).with_args(expected_pvname, expected_value, is_local=True),
                    raises(ValueError))

    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_and_set_pv_raises_first_time_THEN_pv_value_set_called_twice(self, pv_wrapper_mock):
        expected_value = 10
    ...
    @patch("genie_python.genie_epics_api.Wrapper")
    def test_GIVEN_mock_pv_WHEN_set_local_pv_and_set_pv_raises_until_last_attempt_THEN_pv_value_set_called_attempt_times(self, pv_wrapper_mock):
        expected_value = 10
        pv_wrapper_mock.set_pv_value.side_effect = [ValueError(), ValueError(), ValueError(), expected_value]
        pv_name = "PV"
        expected_pvname = "{}{}".format(self.instrument_prefix, pv_name)

        self.api.set_pv_value(expected_pvname, expected_value, is_local=True, attempts=4)
