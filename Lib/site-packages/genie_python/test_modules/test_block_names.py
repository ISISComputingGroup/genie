...
def create_block_names(get_pv_value_mock, blocks, instrument_prefix="Prefix"):
    global block_names_manager
    block_names = BlockNames()
    block_names_manager = BlockNamesManager(block_names, delay_before_retry_add_monitor=1)

    switch_prefix(get_pv_value_mock, block_names_manager, blocks, instrument_prefix)
    return block_names, block_names_manager


def switch_prefix(get_pv_value_mock, blocks_manager, blocks=None, instrument_prefix="Prefix", get_blocks_pv_value=None):
    """
...
class TestGenieAutoCompletePyConsole(unittest.TestCase):
    ...
    def tearDown(self):
        global block_names_manager
        if block_names_manager is not None:
