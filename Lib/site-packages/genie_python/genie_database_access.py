...
SELECT e.experimentID, u.name as userName, r.name as roleName, t.startDate, e.duration FROM `experimentteams` t
...
class GetExperimentData:
    ...
    def get_sql(self):
        ...
        if self._sql is None:
            self._sql = SQLAbstraction(dbid='exp_data', user='report', password='$report', host=self.instrument)
        return self._sql

    def _parameter_is_valid(self, value, column, table):
        """
        ...
        result = self.get_sql().query(command=f'SELECT * FROM `{table}` WHERE {column} = %s',
    ...
    def get_exp_data(self, rb: int = None, user: str = None, role: str = None, verbose: bool = False):
        """
        Returns the data of experiments that match the given criteria, or all if none is given, from the exp_data
        database. If verbose is enabled, just pretty-print the data.
        ...
        if verbose:
            if exp_data:
                ...
                raise NotFoundError(f'Found no experiments that match the given criteria '
    ...
    def _validate_parameters(self, rb, user, role):
        if not self._parameter_is_valid(rb, "experimentID", "experimentteams"):
            raise NotFoundError(f'Found no experiments with RB number {rb}.')
        if not self._parameter_is_valid(user, "name", "user"):
            raise NotFoundError(f'User with name "{user}" was not found. Please make sure the title and name are '
                                f'correct (e.g. "Dr John Smith").')
        if not self._parameter_is_valid(role, "name", "role"):
            roles = ', '.join([x[0] for x in self.get_sql().query_returning_cursor("SELECT name FROM `role`", ())])
    ...
    @staticmethod
    def _create_sql_statement_and_args(rb, role, user):
        """
        ...
        if any(x is not None for x in [rb, user, role]):
            ...
            if rb is not None:
                ...
                args.append(rb)
            if user is not None:
                where_clauses.append('upper(u.name) = upper(%s)')
                args.append(user)
            if role is not None:
                where_clauses.append('upper(r.name) = upper(%s)')
                args.append(role)
