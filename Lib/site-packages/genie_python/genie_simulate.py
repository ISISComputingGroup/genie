        ...
        if populate_with_current_blocks:
            ...
            for ky, vl in initial_block_values.items():
                if isinstance(vl, OrderedDict):
                    self.new_api.set_block_value(ky, vl['value'], vl['runcontrol'], vl['lowlimit'], vl['highlimit'])
                else:
                    self.new_api.set_block_value(ky, vl)

    def __enter__(self):
        print("Entering genie_python simulation mode")
        print("For more information on simulation mode see https://github.com/ISISComputingGroup/ibex_user_manual/wiki/Simulating-Scripts")
