...
from CaChannel._ca import AlarmSeverity, AlarmCondition, dbf_type_to_DBR_STS, dbf_type_to_DBR_TIME
...
class CaChannelWrapper(object):
    ...
    def CAExceptionHandler(epics_args, user_args):
        ...
        Returns:
            None.
        """
        CaChannelWrapper.logError("CAException: ctx={} type={} state={} op={} file={} lineNo={}".format(
            epics_args['ctx'], epics_args['type'], epics_args['state'], epics_args['op'],
            epics_args['file'], epics_args['lineNo']))
    ...
    @staticmethod
    def set_pv_value(name, value, wait=False, timeout=TIMEOUT, safe_not_quick=True):
        """
    ...
    @staticmethod
    def _check_for_disp(name):
        """
        Check if DISP is set on a PV. If passed a field instead of a PV, do nothing.
        Only check DISP if it exists.
        """
        if ".DISP" not in name:  # Do not check for DISP if it's already in the name of the PV to check
            if "." in name:  # If given a field on a PV, check the PV itself if DISP is set
                name = name.split(".")[0]
            _disp_name = "{}.DISP".format(name)
            if CaChannelWrapper.pv_exists(_disp_name, 0) and CaChannelWrapper.get_pv_value(_disp_name) != '0':
    ...
    @staticmethod
    def get_pv_value(name, to_string=False, timeout=TIMEOUT, use_numpy=None):
        """
        ...
        if to_string:
            ...
            if use_numpy is None:
                ...
                return chan.getw(use_numpy=use_numpy)

    @staticmethod
    def get_pv_timestamp(name, timeout=TIMEOUT):
        """
    ...
    @staticmethod
    def add_monitor(name, call_back_function, link_alarm_on_disconnect=True, to_string=False, use_numpy=None):
        """
        ...
        chan.add_masked_array_event(field_type_with_status, count=None,
                                    mask=None, callback=_process_call_back,
                                    use_numpy=use_numpy)
    ...
    @staticmethod
    def _wait_for_pend_event(chan, event, timeout=None, interval=0.1):
        """
        Wait for a pending event to occur in short intervals to allow for keyboard interrupt; has possible timeout for
        maximum time to wait. This should be used for put operation callbacks.
        ...
        while True:
            ...
            # did try flush_io() followed by event.wait(1.0) inside the loop for set pv, but a send got missed
            # (this is what util/caput in CaChannel does with its wait is set to True)
            # So looks like pend_event() / pend_io() / poll() is needed
            # CaChannel example uses pend_event, pyepics seems to do both pend_io and pend_event
            # According to docs, if using preemptive callbacks then only an initial flush_io() should be needed
