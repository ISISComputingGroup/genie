    ...
    def add_monitor(name, call_back_function):
        ...
        Args:
            ...
            call_back_function: the callback function, arguments are value,
                ...
                NB severity and status may not work, these have not yet been tested.
        """
        chan = CaChannelWrapper._connect_channel(name)
        if not chan.read_access():
            raise ReadAccessException(name)
        ...
        def _process_call_back(epics_args, user_args):
            call_back_function(epics_args.get('pv_value', None),
