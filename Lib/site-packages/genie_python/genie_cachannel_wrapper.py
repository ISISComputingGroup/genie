...
class CaChannelWrapper(object):
    ...
    def CAExceptionHandler(epics_args, user_args):
        ...
        Args:
            epics_args (tuple): Contains the results of the action - see CA "struct exception_handler_args"
        ...
        Returns:
            None.
        """
        CaChannelWrapper.logError("CAException: type={} state={} op={} file={} lineNo={}".format(epics_args['type'], epics_args['state'], epics_args['op'], epics_args['file'], epics_args['lineNo']))
    ...
    def set_pv_value(name, value, wait=False, timeout=TIMEOUT):
        ...
        """
        chan = CaChannelWrapper._connect_channel(name)
    ...
    @staticmethod
    def _connect_channel(name):
        """
        If in the cache return the cached object otherwise connect to a channel and cache it.
    ...
    def add_monitor(name, call_back_function):
        ...
        :param call_back_function: the callback function, arguments are value, alarm status, alarm severity
        :return:
        """
        chan = CaChannelWrapper._connect_channel(name)
        if not chan.read_access():
        ...
        def _process_call_back(epics_args, _):
            call_back_function(epics_args['pv_value'], epics_args['pv_status'], epics_args['pv_severity'])

        chan.add_masked_array_event(field_type, count=None, mask=None, callback=_process_call_back)
