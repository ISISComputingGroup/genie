...
class CaChannelWrapper(object):
    ...
    def set_pv_value(name, value, wait=False, timeout=TIMEOUT):
        ...
        if wait:
            ...
            # Looks like pend_event() / pend_io() / poll() is needed
            CaChannelWrapper.pend_io(chan, interval)
    ...
    def connect_to_pv(ca_channel):
        ...
        CaChannelWrapper.pend_io(ca_channel, interval)
        while not event.wait(interval) and time_elapsed <= timeout:
    ...
    @staticmethod
    def pend_io(chan, interval=None):
        """
        Calls underlying channel acces pend_io but returns True/False rather than raise exception

        Args:
            chan: channel to use
            interval: maximun time to wait, None means use default channel access timeout.

        Returns:
            True if processing completes, False if timeout and incomplete
        """
        try:
