...
class ScriptDefinition(object, metaclass=ABCMeta):
    ...
    def run(self, **kwargs):
        ...
        Args:
            **kwargs: The action parameters and their values in a keyword argument format. These must
                have a default value provided as shown in examples e.g. value1: str="1" where "1" is the
                default value.
    ...
    @abstractmethod
    def parameters_valid(self, **kwargs):
        """
    ...
    def estimate_time(self, **kwargs):
        """
        Calculates an estimate in seconds of how long the specified parameters will take to execute.
        The Script Generator only calls this function if all parameters are valid (i.e. parameters_valid returns None)

        Args:
            **kwargs: The action parameters and their values in a keyword argument format. These must
                have a default value provided as shown in the examples e.g. value1: str="1" where "1" is the
                default value.
    ...
    def estimate_custom(self, **kwargs):
        """
        Create custom estimates that display how many events are in the script.
        This function must return an OrderedDict with string keys and string values. The keys are the event names and
        will be displayed as columns in the client.
        The Script Generator only calls this function if all parameters are valid (i.e. parameters_valid returns None)
...
def cast_parameters_to(*args_casts, **keyword_casts) -> Callable[[Callable[[Any], Any]],Callable[[Any], Any]]:
    """
