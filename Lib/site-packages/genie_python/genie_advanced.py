...
from genie_python.mysql_abstraction_layer import SQLAbstraction, Error as sqlError
...
def get_exp_data(rb: int = None, user: str = None, role: str = None):
    ...
    try:
        ...
        sys.stderr.write(f'Error connecting to the database: {e.msg}\n')
        return
    ...
    if any(x is not None for x in [rb, user, role]):
        ...
        if user is not None:
            ...
            if not user_id:
                sys.stderr.write(f'User with name "{user}" was not found. Please make sure the title and name are'
        ...
        if role is not None:
            ...
            if not role_id:
                ...
                sys.stderr.write(f'Role "{role}" was not found. Existing roles: [{roles}]\n')
                return
    ...
    if not records:
        sys.stderr.write(f'Found no experiments that match the given criteria '
