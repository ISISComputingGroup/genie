...
@helparglist('verbose')
@log_command_and_handle_exception
def set_script_dir(base_script_dir, script_dir):
    """
    Allow user to set the genie_python script directory relative to instrument scientist set dir (make the directory if
    it doesn't exist)

    Notes:
      This is usually placed in the init file as:
        def set_dir(new_dir):
            g.adv.set_script_dir(r"<preferred root dir>", new_dir)

        def init(inst):
            set_dir("")
            print("To set a user directory where scripts and data are save use set_dir('<directory name>'), "
                  "e.g. set_dir('Smith2020')")
