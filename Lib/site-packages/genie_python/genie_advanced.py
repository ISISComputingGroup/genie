...
def get_manager_mode() -> bool:
    ...
    manager_mode = __api.get_pv_value("CS:MANAGER", True, 3, True)
    return manager_mode == "Yes"
...
def assert_in_manager_mode() -> None:
    ...
    if not get_manager_mode():
        raise RuntimeError("You need to be in manager mode to be able to complete this action.")

@usercommand
@helparglist('name str, value flt')
def set_motor_position(name: str, value: float | int) -> None:
    ...
    pv_name = __api.inst_prefix + "MOT:" + name

    if not __api.pv_exists(pv_name):
        raise ValueError("Cannot find motor called " + name)
...
@helparglist('pv str')
@log_command_and_handle_exception
def pv_exists(pv, is_local=False):
    """
    Check if PV exists.
...
@usercommand
@helparglist('')
def open_plot_window(is_primary=True, host=None, figures=None):
    """
    Open the plot window in a locally running client (even if this is called in a standalone genie_python)
    Args:
    ...
    """
    from genie_python.matplotlib_backend.ibex_websocket_backend import ibex_open_plot_window, figure_numbers
    ibex_open_plot_window(figures=figure_numbers if figures is None else figures, is_primary=is_primary, host=host)
...
@helparglist('')
@log_command_and_handle_exception
def get_exp_data(rb='%', user='%', role='%', verbose=False):
    """
    ...
    try:
        ...
        raise NotImplementedError("get_exp_data is not implemented for this genie type. {}".format(e))
