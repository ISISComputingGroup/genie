...
# Scipy loads some intel fortran DLLs which install a custom signal handler
# which causes a crash when interrupting a sleep after importing certain scipy
# modules such as scipy.stats or scipy.optimize
#
# >>> import scipy.stats
# >>> sleep(30)
#
# We can tell fortran not to enable this handler by setting an environment variable. See
# https://github.com/ISISComputingGroup/IBEX/issues/3443 for more details including stack traces.
#
# This variable appears to be set to 'T' already by scipy itself, but we set it explicitly
# here in case they decide to no longer set it in a future version.
#
# Note:
#   An alternative solution installs a custom win32 handler, like this:
#
# >>> def handler(sig):
# >>>     try:
# >>>         import _thread
...
#
# However this has the side effect that a sleep cannot be interrupted until it's over, so can not be used.
# See https://github.com/ISISComputingGroup/IBEX/issues/4070 for more details
#
# os.environ['FOR_DISABLE_CONSOLE_CTRL_HANDLER'] = 'T'
print("FORTRAN: {}".format(os.environ.get("os.environ['FOR_DISABLE_CONSOLE_CTRL_HANDLER']", "It wasn't set")))
