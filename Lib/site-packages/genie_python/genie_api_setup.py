...
def log_command_and_handle_exception(_func_name_unlikely_to_be_reused, *args, **kwargs):
    ...
    try:
        ...
        if time_taken < 0.002:
            time_taken = 0.0
        __api.logger.log_command(
            _func_name_unlikely_to_be_reused.__name__,
            log_args,
            command_exception,
            time_taken=time_taken,
        )
        if command_exception is not None:
            __api.logger.log_command_error_msg(
                _func_name_unlikely_to_be_reused.__name__, command_exception
            )
