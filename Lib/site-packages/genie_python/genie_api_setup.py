...
class LoadScriptCompleter:
            ...
            if len(paths) != 0:
                if self.is_pydev:
                    ...
                    if user_script:
                        ...
                        completion = []
                        for ans in paths:
                            # Pydev requires only the completion up to the last splitting char
    ...
    def _get_search_path_and_parameters_from_line_buffer(self, line_buffer):
        """
        Parse the line and get out the path for where to search scripts along with the match, whether we're completing in the user scripts dir 
		and what quote to add to the completion.
        Args:
        ...
        if search_path is None or search_path == "":
            ...
            # full path
            search_path = search_path[1:] # Remove the quote at the front of the path
            quote = ""
            match = search_path
            user_script = False
