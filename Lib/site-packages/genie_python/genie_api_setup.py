...
class LoadScriptCompleter:
    ...
    def complete(self, text=None, line_buffer=None, cursor_pos=None):
        ...
        if LOAD_SCRIPT_COMMAND not in line_buffer:
            ...
            if self.is_pydev:
                ...
                completion = []
                for path in paths:
                    # py dev replaces back to the last splitting character. Find that in the line buffer.
                ...
                completion = [six.u("{}{}".format(quote, path.replace('\\', '/').split(USER_SCRIPT_DIR)[-1]))
                              for path in paths]
    ...
    def _get_search_path_and_parameters_from_line_buffer(self, line_buffer):
        ...
        search_path = search_path.ltrim('"\'')

        if search_path is None:
            search_path = USER_SCRIPT_DIR

        if not os.path.isabs(search_path):
            os.path.join(USER_SCRIPT_DIR, search_path)
