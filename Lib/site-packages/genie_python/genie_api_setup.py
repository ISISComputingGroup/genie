...
class LoadScriptCompleter:
    ...
    def _get_completion_paths(self, line_buffer):
        """
        ...
        Returns:
            tuple (list[str], str, bool) - a list of the completions, the search path which was used,
                whether the path was already quoted, whether the path was absolute (e.g. c:/scripts/myscript.py)
    ...
    def standalone_complete(self, completer, text=None):
        """
    ...
    def pydev_complete(self, text=None, line_buffer=None, cursor_pos=None):
        """
        Find completions for the given text and line context.
...
class PyDevComplete:
    ...
    we need to override this to return just the paths and not the other possible completions.
...
@decorator
def log_command_and_handle_exception(_func_name_unlikely_to_be_reused, *args, **kwargs):
    """
    Decorator that will log the command when run and will
    catch all exceptions to be handled by genie.

    Note: _func_name_unlikely_to_be_reused must be called something that a user will not accidentally
    put into a genie function
    """
    log_args = {"kwargs": kwargs}
    arg_names = getfullargspec(_func_name_unlikely_to_be_reused).args
    ...
    start = time.time()
    try:
        return_value = _func_name_unlikely_to_be_reused(*args, **kwargs)
        return return_value
        ...
        # hack to allow tests to pass on linux. Windows only as millisecond
        # resolution, linux has better, and mock is expecting 0.0
        if time_taken < .001:
            time_taken = 0.0
        __api.logger.log_command(_func_name_unlikely_to_be_reused.__name__, log_args, command_exception,
                                 time_taken=time_taken)
        if command_exception is not None:
            __api.logger.log_command_error_msg(_func_name_unlikely_to_be_reused.__name__, command_exception)
