...
class API(object):
    ...
    def set_instrument(self, pv_prefix, globs, import_instrument_init=True):
        """Set the instrument being used by setting the PV prefix"""
        API.__mod = __import__('init_default', globals(), locals(), [], -1)
        if pv_prefix.startswith("NDX") and import_instrument_init:
    ...
    def set_pv_value(self, name, value, wait=False, attempts=3, is_local=False):
        ...
        Parameters:
            ...
            attempts (int, optional): how many times it retries to set the pv value if it fails
    ...
    def get_pv_value(self, name, to_string=False, attempts=3, is_local=False):
        ...
        Parameters:
            ...
            attempts (int, optional) : how many times to retry getting the pv value if it fails
            is_local (bool, optional) : whether to automatically prepend the local inst prefix to the PV name
        """
        if is_local:
            if not name.startswith(API.__inst_prefix):
