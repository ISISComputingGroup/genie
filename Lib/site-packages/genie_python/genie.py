...
def waitfor(block=None, value=None, lowlimit=None, highlimit=None, maxwait=None,
            ...
            frames=None, raw_frames=None, uamps=None, events=None, mevents=None, early_exit=lambda: False, **pars):
    """
    ...
    Args:
        ...
        events (int, optional): wait for a total number of events to be collected
    ...
    try:
        ...
        __api.waitfor.start_waiting(block, value, lowlimit, highlimit, maxwait, wait_all, seconds, minutes, hours, time,
                                    frames, raw_frames, uamps, events, mevents, early_exit)
    except Exception as e:
        _handle_exception(e)
...
@usercommand
@helparglist('events')
def waitfor_events(events):
    """
    Interrupts execution to wait for number of events to reach parameter value

    Args:
        events (int): the number of events to wait for
    ...
    try:
        if events is None:
            raise TypeError("Cannot execute waitfor_events - need to set events parameter. Type help(waitfor_events")
        if events < 0:
            raise ValueError("Cannot execute waitfor_events - events parameter cannot be negative")
        if __api.waitfor is None:
            raise Exception("Cannot execute waitfor_events - try calling set_instrument first")
        __api.waitfor.start_waiting(events=events)
    except Exception as e:
        _handle_exception(e)
