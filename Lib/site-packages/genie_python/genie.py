...
def cset(*args, **kwargs):
    ...
    try:
        ...
        value = None
        values = []
        runcontrol = kwargs.pop('runcontrol', None)
        lowlimit = kwargs.pop('lowlimit', None)
        highlimit = kwargs.pop('highlimit', None)
        ...
        if len(args) > 0:
            ...
            if not __api.block_exists(args[0]):
                raise Exception('No block with the name "{}" exists'.format(args[0]))
            else:
                block = args[0]
            if len(args) == 2:
                value = args[1]
        ...
        for k in kwargs:
            ...
            if __api.block_exists(k):
                ...
                raise Exception('No block with the name "{}" exists'.format(k))
