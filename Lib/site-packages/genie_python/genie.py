...
def _log_alarmed_block(block_name, alarm_state):
    _genie_api.log_info_msg("BLOCK {} IN {} ALARM".format(block_name, alarm_state))
...
def waitfor(block=None, value=None, lowlimit=None, highlimit=None, maxwait=None,
    ...
    _genie_api.waitfor.start_waiting(block, value, lowlimit, highlimit, maxwait, wait_all, seconds, minutes, hours,
                                     time, frames, raw_frames, uamps, mevents, early_exit)
...
def waitfor_time(seconds=None, minutes=None, hours=None, time=None):
    ...
    _genie_api.waitfor.start_waiting(seconds=seconds, minutes=minutes, hours=hours, time=time)
...
def waitfor_frames(frames):
    ...
    _genie_api.waitfor.start_waiting(frames=frames)
...
def waitfor_raw_frames(raw_frames):
    ...
    if raw_frames is None:
        raise TypeError(
            "Cannot execute waitfor_raw_frames - need to set raw_frames parameter. Type help(waitfor_raw_frames")
    if raw_frames < 0:
    ...
    _genie_api.waitfor.start_waiting(raw_frames=raw_frames)
...
def waitfor_uamps(uamps):
    ...
    _genie_api.waitfor.start_waiting(uamps=uamps)
...
def waitfor_mevents(mevents):
    ...
    _genie_api.waitfor.start_waiting(mevents=mevents)
...
def begin(period=1, meas_id=None, meas_type="", meas_subid="",
    ...
    pre_post_cmd_return = _genie_api.pre_post_cmd_manager.begin_precmd(quiet=quiet)
    if pre_post_cmd_return is None:
        _genie_api.dae.begin_run(period, meas_id, meas_type, meas_subid, sample_id, delayed, quiet, paused)

        waitfor_runstate("SETUP", onexit=True)

        _genie_api.dae.post_begin_check(verbose)
        _genie_api.pre_post_cmd_manager.begin_postcmd(run_num=_genie_api.dae.get_run_number(), quiet=quiet)
    else:
...
def abort(verbose=False):
    ...
    Args:
        verbose (bool, optional): show the messages from the DAE
    """
    _genie_api.pre_post_cmd_manager.abort_precmd()
    _genie_api.dae.abort_run()
    _genie_api.dae.post_abort_check(verbose)
    _genie_api.pre_post_cmd_manager.abort_postcmd()
...
def end(verbose=False, quiet=False, immediate=False):
    ...
    """
    _genie_api.pre_post_cmd_manager.end_precmd(quiet=quiet)
    _genie_api.dae.end_run(verbose=verbose, quiet=quiet, immediate=immediate)
    waitfor_runstate("SETUP")
    _genie_api.dae.post_end_check(verbose)
    _genie_api.pre_post_cmd_manager.end_postcmd(quiet=quiet)
...
def pause(verbose=False, immediate=False):
    ...
    """
    _genie_api.pre_post_cmd_manager.pause_precmd()
    _genie_api.dae.pause_run(immediate=immediate)
    _genie_api.dae.post_pause_check(verbose)
    _genie_api.pre_post_cmd_manager.pause_postcmd()
...
def resume(verbose=False):
    ...
    Args:
        verbose (bool, optional): show the messages from the DAE
    """
    _genie_api.pre_post_cmd_manager.resume_precmd()
    _genie_api.dae.resume_run()
    _genie_api.dae.post_resume_check(verbose)
    _genie_api.pre_post_cmd_manager.resume_postcmd()
...
def enable_hard_periods(mode, period_file=None, sequences=None, output_delay=None, period=None, daq=False, dwell=False,
    ...
    """
    _genie_api.dae.configure_hard_periods(mode, period_file, sequences, output_delay, period, daq, dwell, unused,
                                          frames, output, label)
...
def configure_internal_periods(sequences=None, output_delay=None, period=None, daq=False, dwell=False, unused=False,
    ...
    Note: if the period number is unspecified then the settings will be applied to all periods
    """
    _genie_api.dae.configure_internal_periods(sequences, output_delay, period, daq, dwell, unused, frames, output,
                                              label)
