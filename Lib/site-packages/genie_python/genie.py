...
from genie_python.utilities import EnvironmentDetails, UserToggleSettings, get_correct_path, \
...
def cset(*args, **kwargs):
    ...
    if verbose is not False and (verbose or UserToggleSettings.cset_verbose):
        for block, value in list(zip(blocks, values)):
...
@helparglist('...')
@log_command_and_handle_exception
def toggle_cset_verbose(verbose):
    """
    ...
    Examples:
        ...
        >>> toggle_cset_verbose(True)
    """
    if not isinstance(verbose, bool):
        raise ValueError('Default verbosity needs to be True or False.')
    UserToggleSettings.cset_verbose = verbose
    print('Default cset verbosity set to {}.'.format(verbose))
...
def waitfor(block=None, value=None, lowlimit=None, highlimit=None, maxwait=None,
    ...
    _genie_api.waitfor.start_waiting(block, value, lowlimit, highlimit, maxwait, wait_all, seconds, minutes, hours, time,
...
def enable_hard_periods(mode, period_file=None, sequences=None, output_delay=None, period=None, daq=False, dwell=False,
    ...
    """
    _genie_api.dae.configure_hard_periods(mode, period_file, sequences, output_delay, period, daq, dwell, unused, frames,
                                          output, label)
