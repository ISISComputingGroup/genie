...
def cget(block):
    ...
    """
    ans = __api.get_block_data(block)
...
def _log_alarmed_block(block_name, alarm_state):
    __api.log_info_msg("BLOCK {} IN {} ALARM".format(block_name, alarm_state))
...
def cshow(block=None):
    ...
    Examples:
        ...
        >>> cshow("block1")
    """
    blocks_to_get = [block] if block is not None else __api.get_block_names()
    for block in blocks_to_get:
        _print_from_cget(__api.get_block_data(block, True))
...
def get_dashboard():
    ...
    data["raw_frames_period"] = __api.dae.get_raw_frames(True)
