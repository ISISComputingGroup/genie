...
def begin(period=1, meas_id=None, meas_type="", meas_subid="", sample_id="", delayed=False, quiet=False, paused=False,
    ...
    try:
        ...
        return __api.run_pre_post_cmd("begin_postcmd", run_num=__api.dae.get_run_number(), quiet=quiet)
    except Exception as e:
        _handle_exception(e)
...
@usercommand
@helparglist('[verbose]')
def end(verbose=False, quiet=False):
    """
    End the current run.
    ...
    __api.log_command(sys._getframe().f_code.co_name, locals())
    try:
        __api.run_pre_post_cmd("end_precmd", quiet=quiet)
        __api.dae.end_run()
