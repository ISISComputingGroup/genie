...
from genie_python.utilities import get_correct_path, get_correct_filepath_existing
    ...
    try:
        ...
        try:
            try:
                ...
                full_name = get_correct_filepath_existing(os.path.join(USER_SCRIPT_DIR, name))
        except Exception:
...
def change_tables(wiring=None, detector=None, spectra=None):
    ...
    Polls for all of the tables to write correctly in the dae,
    throws exception if any table fails to write.
    ...
    __api.log_command(sys._getframe().f_code.co_name, locals())
    try:
        #create a dictionary to hold the Table type, the path and the write state in.
        paths_to_check = {}
        if wiring is not None:
            paths_to_check["Wiring"] = [get_correct_filepath_existing(wiring), False]
        if spectra is not None:
            paths_to_check["Spectra"] = [get_correct_filepath_existing(spectra), False]
        if detector is not None:
            paths_to_check["Detector"] = [get_correct_filepath_existing(detector), False]

        if paths_to_check:#list is not empty, we have some tables to change

            __api.dae.change_tables(wiring, detector, spectra)
            now = datetime.datetime.now() 
            timeout = now + datetime.timedelta(minutes=1)
            written = True
            
            #basic one minute timeout loop
            while(datetime.datetime.now() < timeout):
                for key, item in paths_to_check.iteritems():
                    if __api.dae.get_table_path(key) == item[0]:
                        item[1] = True
                        written = written & item[1] #see if all tables write
                if written: #if they all write, exit timeout loop
                    break
            ...
            print("No file paths were provided.") # should this be an exception?
    except Exception as e:
        _handle_exception(e)
