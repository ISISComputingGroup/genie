...
def abort(verbose=False):
    ...
    try:
        ...
        __api.dae.abort_run()
        # this runstate check implementation is faster than the existing
        # waitfor_runstate() however the check could likely be skipped
        # as post_check should catch any error
        if __api.dae.get_run_state() != "SETUP":
            print("Failed to ABORT", file=sys.stdout)
            self.api.log_error_msg("RUN NOT SETUP")
...
def pause(verbose=False):
    ...
    try:
        ...
        # waitfor_runstate() however the check could likely be skipped
        # as post_check should catch any error
        if __api.dae.get_run_state() != "PAUSED":
            print("Failed to PAUSE", file=sys.stdout)
            self.api.log_error_msg("RUN NOT PAUSED")
...
def resume(verbose=False):
    ...
    try:
        ...
        # as post_check should catch any error
        if __api.dae.get_run_state() == "PAUSED":
            print("Failed to RESUME", file=sys.stdout)
            self.api.log_error_msg("RUN NOT RESUMED")
