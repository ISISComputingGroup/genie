...
SUPPORTED_PYTHON_VERSION = (3, 8, 6)
...
@helparglist('block')
@log_command_and_handle_exception
def get_block_units(block_name):
    """
...
def _log_alarmed_block(block_name, alarm_state):
...
def _print_from_cget(block_details):
    """
...
def waitfor(block=None, value=None, lowlimit=None, highlimit=None, maxwait=None,
            ...
            frames=None, raw_frames=None, uamps=None, mevents=None, early_exit=lambda: False, silent=False, **pars):
    """
    ...
    Args:
        ...
        silent (bool, optional): suppress normal output messages to the console
    ...
    _genie_api.waitfor.start_waiting(block, value, lowlimit, highlimit, maxwait, wait_all, seconds, minutes, hours,time, frames, raw_frames, uamps, mevents, early_exit, silent)


@usercommand
@helparglist('block[, value][, lowlimit][, highlimit][, maxwait]')
@log_command_and_handle_exception
def waitfor_block(block, value=None, lowlimit=None, highlimit=None, maxwait=None, early_exit=lambda: False, silent=False):
    """
    ...
    _genie_api.waitfor.start_waiting(block=block, value=value, lowlimit=lowlimit, highlimit=highlimit, maxwait=maxwait,
                                     early_exit=early_exit, silent=silent)


@usercommand
@helparglist('[seconds][, minutes][, hours][, time]')
@log_command_and_handle_exception
def waitfor_time(seconds=None, minutes=None, hours=None, time=None, silent=False):
    """
    ...
    _genie_api.waitfor.start_waiting(seconds=seconds, minutes=minutes, hours=hours, time=time, silent=silent)


@usercommand
@helparglist('frames')
@log_command_and_handle_exception
def waitfor_frames(frames, silent=False):
    """
    ...
    _genie_api.waitfor.start_waiting(frames=frames, silent=silent)


@usercommand
@helparglist('raw_frames')
@log_command_and_handle_exception
def waitfor_raw_frames(raw_frames, silent=False):
    """
    ...
    _genie_api.waitfor.start_waiting(raw_frames=raw_frames, silent=silent)


@usercommand
@helparglist('uamps')
@log_command_and_handle_exception
def waitfor_uamps(uamps, silent=False):
    """
    ...
    _genie_api.waitfor.start_waiting(uamps=uamps, silent=silent)


@usercommand
@helparglist('mevents')
@log_command_and_handle_exception
def waitfor_mevents(mevents, silent=False):
    """
    ...
    _genie_api.waitfor.start_waiting(mevents=mevents, silent=silent)


@usercommand
@helparglist('state[, maxwaitsecs][, onexit]')
@log_command_and_handle_exception
def waitfor_runstate(state, maxwaitsecs=3600, onexit=False, silent=False):
    """
    ...
    _genie_api.waitfor.wait_for_runstate(state, maxwaitsecs, onexit, silent=silent)
...
def get_pv(name, to_string=False, is_local=False):
    ...
    """
    return _genie_api.get_pv_value(name, to_string, is_local=is_local)
...
def begin(period=1, meas_id=None, meas_type="", meas_subid="",
          sample_id="", delayed=False, quiet=False, paused=False,
          verbose=False, prepost=True):
    """
...
@log_command_and_handle_exception
def abort(verbose=False, prepost=True):
    """
...
@log_command_and_handle_exception
def end(verbose=False, quiet=False, immediate=False, prepost=True):
    """
    End the current run.
...
@log_command_and_handle_exception
def pause(verbose=False, immediate=False, prepost=True):
    """
...
@helparglist('[verbose], [prepost]')
@log_command_and_handle_exception
def resume(verbose=False, prepost=True):
    """
...
@helparglist('')
@log_command_and_handle_exception
def get_time_since_begin(get_timedelta=False):
    """
...
def change_tables(wiring=None, detector=None, spectra=None):
    ...
    if wiring is not None:
        wiring = get_file(wiring)
    if spectra is not None:
        spectra = get_file(spectra)
    if detector is not None:
        detector = get_file(detector)
