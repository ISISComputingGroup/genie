...
if sys.version_info[0:3] != SUPPORTED_PYTHON_VERSION[0:3]:
    message = "WARNING: genie_python only guarantees support for Python version {0[0]}.{0[1]}.{0[2]}, you are running {1}".format(
...
def _print_from_cget(block_details):
    ...
    """
    format_string = (
        "{name} = {value} (runcontrol = {runcontrol}, lowlimit = {lowlimit}, highlimit = {highlimit}, "
        "alarm = {alarm})"
    )
...
def waitfor_time(seconds=None, minutes=None, hours=None, time=None, quiet=False):
    ...
    if all(t is None for t in (seconds, minutes, hours, time)):
        raise TypeError(
            "Cannot execute waitfor_time - need to set at least one parameter. Type help(waitfor_time) "
            "to see guidelines"
        )
...
def waitfor_mevents(mevents, quiet=False):
    ...
    if mevents is None:
        raise TypeError(
            "Cannot execute waitfor_mevents - need to set mevents parameter. Type help(waitfor_mevents)"
        )
    if mevents < 0:
...
def load_script(name, check_script=True, warnings_as_error=False):
    ...
    try:
        ...
        if len(scripts) > 0:
            ...
            raise Exception(
                "No scripts found in {} - please ensure all your code is contained within functions.".format(
                    file_path
                )
            )
...
def define_hard_period(
    period=None, daq=False, dwell=False, unused=False, frames=None, output=None, label=None
):
    """
