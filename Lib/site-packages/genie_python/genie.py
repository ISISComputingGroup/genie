...
def cget(block):
    ...
    try:
        ...
        if not __api.block_exists(block):
            ...
            if block.upper() in (existing_block.upper() for existing_block in __api.get_blocks()):
                ans['connected'] = False
            else:
                # Can't find block at all
                raise Exception("No block with the name '{}' exists\nCurrent blocks are {}"
                                .format(block, __api.get_blocks()))

        ans['name'] = block
        ans['value'] = __api.get_block_value(block) if ans['connected'] else None
        ans['runcontrol'], ans['lowlimit'], ans['highlimit'] = __api.get_runcontrol_settings(block)

        alarm = __api.get_alarm_from_block(block)
        ans['alarm'] = alarm
        if alarm != "NO_ALARM":
            _log_alarmed_block(block, alarm)
...
def _print_from_cget(block_details):
    ...
    Args:
        block_details (dict): dict containing information on the block (see return of cget)
            ...
            highlimit - run control high limit set
    """
    format_string = '{name} = {value} (runcontrol = {runcontrol}, lowlimit = {lowlimit}, highlimit = {highlimit}, alarm = {alarm})'
