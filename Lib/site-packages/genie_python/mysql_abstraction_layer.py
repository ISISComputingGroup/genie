...
class SQLAbstraction:
    ...
    def __init__(self, dbname, user, password, host="127.0.0.1"):
        """
        Constructor.
        Args:
            dbname (string): The name of the database that holds the required information
    ...
    def _generate_pool_name(self):
        ...
        A connection in the pool is made with the first set of credentials passed, so we
        have to make sure a pool name is not used with different credentials.
        """
        return "DBSVR_%s_%s_%s" % (self._host, self._dbname, self._user)

    def _start_connection_pool(self):
        """
        Initialises a connection pool.
        """
        # print("Creating a new connection pool: " + self._pool_name)
        conn = mysql.connector.connect(user=self._user, password=self._password, host=self._host, database=self._dbname,
        ...
        if len(curs.fetchall()) == 0:
            ...
            raise Exception("Requested Database %s does not exist" % self._dbname)
        curs.close()
        conn.close()
    ...
    def _get_connection(self):
        try:
            ...
            try:
                ...
                msg = err
            raise Exception("Unable to get connection from pool: %s" % msg)

    def execute_command(self, command, is_query, bound_variables):
        """
        ...
        Args:
            ...
            bound_variables (tuple): parameters to be used for the given statement
        Returns:
        ...
        try:
            ...
            print("Error executing command on database: {0}".format(err), "MAJOR")
    ...
    def query_returning_cursor(self, command, bound_variables):
        ...
        try:
            ...
            # Commit as part of the query or results won't be updated between subsequent transactions.
            # Can lead to values not auto-updating in the GUI.
            conn.commit()
        except Exception as err:
            print("Error executing command on database: {0}".format(err), "MAJOR")
            raise DatabaseError(str(err))
