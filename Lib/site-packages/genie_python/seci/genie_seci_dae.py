...
class Dae(object):
    def __init__(self, api, prefix="", session=None):
        """The constructor"""
        self.api = api
    ...
    def get_spectrum(self, spectrum, period=1, dist=False):
        ...
        data = self.dcom_session.getSpectrum(spectrum, period, dist)
        if dist:
            return {'time': data[1], 'signal': data[2], 'sum': data[3], 'mode': 'distribution'}
        else:
            return {'time': data[1], 'signal': data[2], 'sum': data[3], 'mode': 'not dist'}
