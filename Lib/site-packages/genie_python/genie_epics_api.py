...
class API(object):
    ...
    def _setup_logger(self):
        ...
        self.file_handler = logging.FileHandler(f_name, delay=True) # Delay opening the file until the first call to emit() in case file is not available (e.g. on VHD build)
    ...
    def set_instrument(self, machine_identifier, globs, import_instrument_init=True):
        ...
        if not six.PY2:
            try:
                API.exp_data = GetExperimentData(machine)
            except Exception as e:
