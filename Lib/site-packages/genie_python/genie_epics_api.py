...
class API(object):
    ...
    def _setup_logger(self):
        ...
        class instrumentFilter(logging.Filter):
            ...
            def filter(self, record):
                ...
                record.command_response_time = self.command_response_time
                record.exception_text = self.exception_text
                record.from_channel_access = self.from_channel_access
                self.reset()
                return True
    ...
    def gen_log_text(self, source, levelname, message):
        ...
        # "([date] + [time]) ([source]) ([PID]) ([level]) [log message]"
        
        curr_time = localtime()
        t_stamp = strftime("%Y-%m-%dT%H:%M:%S", curr_time)
        log_output = f"({t_stamp})\t({source})\t({os.getpid()})\t({levelname})\t{message}\n"
        return log_output
