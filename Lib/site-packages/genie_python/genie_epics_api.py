...
class API(object):
    ...
    def init_instrument(self, instrument, machine_name, globs, import_instrument_init ):
        ...
        :param instrument: instrument name to load from
        :param machine_name: machine name
        :param globs: globs area in which to put the systems
        :param import_instrument_init (bool): if True import the instrument init from the config area; otherwise don't
        :return: nothing
        """
        instrument = instrument.lower().replace("-", "_")
        print("Loading instrument scripts from: {}".format(
            os.path.join("C:\\", "Instrument", "Settings", "config", machine_name, "Python", "inst")))
