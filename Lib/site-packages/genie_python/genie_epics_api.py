...
from genie_python.utilities import crc8, EnvironmentDetails, remove_field_from_pv, dehex_decompress_and_dejson
from genie_python.genie_logging import GenieLogger, filter as logging_filter
...
class API(object):
    def __init__(self, pv_prefix, globs, environment_details=None):
        ...
        if os.name == "nt":
            ...
            # on linux libCom.so symbols are exposed by CaChannel library
            comdll = "_ca.cpython-310-x86_64-linux-gnu.so"
        try:
    ...
    def _get_machine_details_from_identifier(self, machine_identifier):
        """
        Gets the details of a machine by looking it up in the instrument list first. If there is no match
        it calculates the details as usual.
        ...
        # then find the first match where pvPrefix equals the machine identifier that's been passed to this function
        # if it is not found instrument_details will be None
        instrument_list = dehex_decompress_and_dejson(self.get_pv_value("CS:INSTLIST"))
        instrument_details = next((inst for inst in instrument_list if inst["pvPrefix"] == machine_identifier), None)
    ...
    def set_instrument(self, machine_identifier, globs, import_instrument_init=True):
        ...
        if not six.PY2:
            try:
                ...
                error_message = "Could not connect to database, RB numbers will not be accessible: {}".format(e)
    ...
    def _get_pv_prefix(self, instrument, is_instrument):
        ...
        return "{prefix}:{instrument}:".format(prefix=pv_prefix_prefix, instrument=self.instrument_name)
    ...
    def init_instrument(self, instrument, machine_name, globs, import_instrument_init):
        ...
        if import_instrument_init:
            ...
            python_config_area = os.path.join("C:"+os.sep, "Instrument", "Settings", "config", machine_name, "Python")
            ...
            if six.PY2:
                ...
                self.localmod = __import__('init_' + instrument, globals(), locals(), ['init_' + instrument], -1)
            else:
    ...
    def pv_exists(self, name, fail_fast=False, is_local=False):
        ...
        Args:
            ...
            fail_fast (bool, optional): if True the function will not attempt to wait for a disconnected PV
            is_local (bool, optional): whether to automatically prepend the local inst prefix to the PV name 
    ...
    def connected_pvs_in_list(self, pv_list, is_local=False):
        """
       ...
       # make the mapping work with correct params for def pv_exists(self, name, fail_fast=False, is_local=False): to avoid this eror: def pv_exists(self, name, fail_fast=False, is_local=False):
        def pv_exists_wrapper(name):
        ...
        if names is not None:
            for n in names:
                ...
                if m is not None:
                    ...
                    self.logger.log_error_msg("Unexpected PV found whilst retrieving parameters: {0}".format(n))
        return ans
    ...
    def send_sms(self, phone_num, message):
        ...
        """
        try:
            self.logger.log_info_msg("send_sms returned {}".format(self.sms.send_sms(phone_num, message)))
        except Exception as e:
    ...
    def send_email(self, address, message):
        ...
        """
        try:
            self.logger.log_info_msg("send_email returned {}".format(self.sms.send_email(address, message)))
        except Exception as e:
    ...
    def send_alert(self, message, inst):
        ...
        try:
            self.logger.log_info_msg("send_alert returned {}".format(self.sms.send_alert(inst, message)))
        except Exception as e:
