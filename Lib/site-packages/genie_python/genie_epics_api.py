...
class API(object):
    ...
    def block_exists(self, name):
        ...
        """
        return self.pv_exists(self.get_pv_from_block(name))
    ...
    def set_block_value(self, name, value=None, runcontrol=None, lowlimit=None, highlimit=None, wait=False):
        ...
        if lowlimit is not None:
            self.set_pv_value(full_name + RC_ENABLE, lowlimit)
    ...
    def get_runcontrol_settings(self, block_pv):
        """
        Gets the current run-control settings for a block.

        Args:
            block_pv: the full pv of the block

        Returns:
            tuple: (enabled, low_limit, high_limit)
        """
        try:
            enabled = self.get_pv_value(block_pv + RC_ENABLE) == 1
    ...
    def get_pv_alarm(self, pv_name):
        ...
        """
        try:
            return self.get_pv_value("{}.SEVR".format(utilities.remove_field_from_pv(pv_name)), to_string=True)
        except Exception:
            return "UNKNOWN"
