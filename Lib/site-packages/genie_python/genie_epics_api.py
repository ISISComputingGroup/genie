...
from genie_python.version import from_ibex, VERSION
...
class API(object):
    ...
    exp_data = None  # type: GetExperimentData
    sms = smslib.sms
    __inst_prefix = ""
    __instrument_name = ""
    __machine_name = ""
    ...
    def __init__(self, pv_prefix, globs, environment_details=None):
        ...
        if os.name == "nt":
            ...
            comdll = "_ca.cpython-38-x86_64-linux-gnu.so"
        try:
    ...
    def _setup_logger(self):
        #setup logging for graylog
        instrument_name = self.get_instrument()
        ...
        class instrumentFilter(logging.Filter):
            ...
            def filter(self, record):
                ...
                record.from_ibex = str(self.from_ibex)
            ...
            def reset(self):
                ...
                self.from_ibex = from_ibex
                

        self.genie_logger = logging.getLogger('genie_python_graylogger')
        self.genie_logger.setLevel(logging.DEBUG)

        self.web_handler = graypy.GELFUDPHandler('localhost', 12201)
        self.genie_logger.addHandler(self.web_handler)

        curr_time = localtime()
        if not os.path.exists(API.__log_dir):
            os.makedirs(API.__log_dir)
        current_date_time = strftime("%Y-%m-%d-%a", curr_time)
        f_name = self._get_log_file_name(API.__log_dir, current_date_time)
        self.file_handler = logging.FileHandler(f_name)
        self.genie_logger.addHandler(self.file_handler)

        self.genie_filter = instrumentFilter()
        self.genie_logger.addFilter(self.genie_filter)

    @staticmethod
    def _get_log_file_name(log_dir, current_date_time):
        return log_dir + 'genie-' + current_date_time + '.log'
    ...
    def set_instrument(self, machine_identifier, globs, import_instrument_init=True):
        ...
        self.genie_filter.instrument = instrument
    ...
    def log_info_msg(self, message):
        self.genie_logger.info(self.gen_log_text("CMD", "INFO", message))

    def log_command(self, function_name, arguments, time_taken=None):
        self.genie_filter.command_called = function_name
        self.genie_filter.function_args = arguments
        if time_taken is not None:
            self.genie_filter.time_taken = time_taken
        self.genie_logger.debug(self.gen_log_text("CMD", "DEBUG", f"{function_name} {arguments}"))

    def log_error_msg(self, error_msg):
        """Log the error to the log file"""
        self.genie_filter.exception_text = error_msg
        self.genie_logger.error(self.gen_log_text("CMD", "ERROR", error_msg))

    def log_ca_msg(self, error_msg):
        """Log the CA error (from CaChannel) to the log file"""
        self.genie_filter.exception_text = error_msg
        self.genie_logger.error(self.gen_log_text("CA", "ERROR", error_msg))

    def gen_log_text(self, source, levelname, message):
        # generates the correct format of log text, in the format
        # "[date]T[time] ([source]) ([PID]) [log message] ([level])"
        
        curr_time = localtime()
        t_stamp = strftime("%Y-%m-%dT%H:%M:%S", curr_time)
        log_output = f"{t_stamp}\t({source})\t({os.getpid()})\t{message}\t({levelname})"
        return log_output
