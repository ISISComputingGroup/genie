...
class API(object):
    def __init__(self, pv_prefix, globs, environment_details=None):
        ...
        if os.name == "nt":
            ...
            sys.version_info[0]
            comdll = f"_ca.cpython-{sys.version_info[0]}{sys.version_info[1]}-x86_64-linux-gnu.so"
        try:
    ...
    def set_block_value(
        self, name, value=None, runcontrol=None, lowlimit=None, highlimit=None, wait=False
    ):
        """
        ...
        if lowlimit is not None and highlimit is not None:
            if lowlimit > highlimit:
                print(
                    "Low limit ({}) higher than high limit ({}), swapping them around for you".format(
                        lowlimit, highlimit
                    )
                )
    ...
    def send_sms(self, phone_num, message):
        ...
        try:
            self.logger.log_info_msg(
                "send_sms returned {}".format(self.sms.send_sms(phone_num, message))
            )
        except Exception as e:
    ...
    def send_email(self, address, message):
        ...
        try:
            self.logger.log_info_msg(
                "send_email returned {}".format(self.sms.send_email(address, message))
            )
        except Exception as e:
    ...
    def send_alert(self, message, inst):
        ...
        try:
            self.logger.log_info_msg(
                "send_alert returned {}".format(self.sms.send_alert(inst, message))
            )
        except Exception as e:
