...
from genie_python.genie_api_setup import __api, helparglist, usercommand
...
# current delay is global, but will be changing
_ALERT_DELAY_OUT = "CS:AC:OUT:DELAY"
_ALERT_DELAY_IN = "CS:AC:IN:DELAY"
...
def set_range(block, lowlimit, highlimit, delay_in=None, delay_out=None):
    """
    ...
    Args:
        ...
        delay_in (float): (optional) delay /s before triggering in range
...
def _print_block(block, only_if_enabled=False):
    enabled = __api.get_pv_value(_ALERT_ENABLE.format(block), to_string=False, is_local=True)
    if only_if_enabled and not enabled:
        return
    print("Block: {}".format(block))
    print("  Enabled:   {}".format(enabled))
    print("  Low:       {}".format(__api.get_pv_value(_ALERT_LOW.format(block), to_string=False, is_local=True)))
    print("  High:      {}".format(__api.get_pv_value(_ALERT_HIGH.format(block), to_string=False, is_local=True)))
    print("  Delay In:  {}".format(__api.get_pv_value(_ALERT_DELAY_IN.format(block), to_string=False, is_local=True)))
    print("  Delay Out: {}".format(__api.get_pv_value(_ALERT_DELAY_OUT.format(block), to_string=False, is_local=True)))
...
def _dump(block):
    ...
    res['delay_in'] = __api.get_pv_value(_ALERT_DELAY_IN.format(block), to_string=False, is_local=True)
    res['delay_out'] = __api.get_pv_value(_ALERT_DELAY_OUT.format(block), to_string=False, is_local=True)
    return res
