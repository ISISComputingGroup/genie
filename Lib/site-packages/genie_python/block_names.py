...
# lock used to access _timer or _pv_name or _pv_monitored
PV_NAME_LOCK = RLock()

# Event to indicate that adding a monitor should no longer be scheduled
STOP_SCHEDULE = Event()
...
class BlockNamesManager:
    ...
    def __init__(self, block_names, delay_before_retry_add_monitor=DELAY_BEFORE_RETRYING_BLOCK_NAMES_PV_ON_FAIL):
        ...
        self._timer = None
        self._pv_name = None
        self._pv_monitored = None
        STOP_SCHEDULE.clear()
        atexit.register(self.stop_add_monitor)
    ...
    def update_prefix(self, pv_prefix):
        ...
        with PV_NAME_LOCK:
            self._pv_name = "{}{}{}".format(pv_prefix, BLOCK_SERVER_PREFIX, PV_BLOCK_NAMES)
    ...
    def _add_monitor(self):
        ...
        with PV_NAME_LOCK:
            self._timer = None
            pv_name_in_lock = self._pv_name

            if self._pv_monitored == pv_name_in_lock:
                # monitoring the correct pv
                return
        ...
        try:
            self._cancel_monitor_fn = CaChannelWrapper.add_monitor(pv_name_in_lock, self._update_block_names,
                                                                   to_string=True)
            with PV_NAME_LOCK:
                self._pv_monitored = pv_name_in_lock
        except UnableToConnectToPVException:
            pass
        else:
            # get initial value
            try:
                self._update_block_names(CaChannelWrapper.get_pv_value(pv_name_in_lock, to_string=True), None, None)
            except UnableToConnectToPVException:
                # if pv is not available then monitor will pick it up when it changes
                pass

        # Schedule next add monitor if needed; i.e. a old pv-prefix change was slower the the last pv-prefix and so we
        # are monitoring the wrong pv.
        with PV_NAME_LOCK:
            if not STOP_SCHEDULE.isSet() and self._pv_name != self._pv_monitored:
                self._timer = Timer(self._delay_before_retry_add_monitor, self._add_monitor)
                self._timer.start()

    def stop_add_monitor(self):
        """
        Stops the chain of trying to add a monitor
        """
        STOP_SCHEDULE.set()
        with PV_NAME_LOCK:
