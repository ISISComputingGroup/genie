...
MINUTES_UNTIL_RENOTIFICATION_OF_EXCEPTION = 5
...
class WaitForControllerState:
    ...
    @abstractmethod
    def pv_connected(self):
        pass


class WaitForControllerNotifyExceptionState(WaitForControllerState):
    """
    The state for when the WaitForController is in an exception state and should notify the user of an exception
    """

    def handle_exception_notification(self, exception):
        print("{}: Exception in waitfor loop: {}: {}".format(str(datetime.utcnow()),
                                                             exception.__class__.__name__, exception))
        self._api.log_info_msg("EXCEPTION IN WAITFOR, PV MAY BE DISCONNECTED")
        self.context.transition_to(WaitForControllerSuppressNotifyExceptionState(self._api,
                                                                                 datetime.utcnow(), exception))

    def pv_connected(self):
        self.context.transition_to(WaitForControllerNotifyReconnectState(self._api, datetime.utcnow(),
                                                                         self._last_exception))


class WaitForControllerSuppressNotifyExceptionState(WaitForControllerState):
    """
    The state for when the WaitForController is in an exception state but should not notify the user
    """

    def handle_exception_notification(self, exception):
        minutes_since_last_notification = ((datetime.utcnow() - self._last_notification_time).seconds // 60) % 60
        if minutes_since_last_notification >= MINUTES_UNTIL_RENOTIFICATION_OF_EXCEPTION:
            self.context.transition_to(WaitForControllerNotifyExceptionState(self._api,
                                                                             self._last_notification_time, exception))
        if not isinstance(exception, type(self._last_exception)):
            self.context.transition_to(WaitForControllerNotifyExceptionState(self._api,
                                                                             self._last_notification_time, exception))

    def pv_connected(self):
        self.context.transition_to(WaitForControllerNotifyReconnectState(self._api, self._last_notification_time,
                                                                         self._last_exception))


class WaitForControllerNotifyReconnectState(WaitForControllerState):
    """
    The state for when the WaitForController has just reconnected after an exception state
    """

    def handle_exception_notification(self, exception):
        self.context.transition_to(WaitForControllerNotifyExceptionState(self._api, self._last_notification_time,
                                                                         self._last_exception))

    def pv_connected(self):
        print("{}: PV reconnected".format(str(datetime.utcnow())))
        self.context.transition_to(WaitForControllerSuppressNotifyReconnectState(self._api,
                                                                                 self._last_notification_time,
                                                                                 self._last_exception))


class WaitForControllerSuppressNotifyReconnectState(WaitForControllerState):
    """
    The state for when the WaitForController is not in an exception state and has not just reconnected
    """
...
class WaitForControllerExceptionContext(object):
    ...
    def exception_occured(self, exception):
        self._state.handle_exception_notification(exception)

    def pv_connected(self):
        self._state.pv_connected()
...
class WaitForController(object):
    ...
    def start_waiting(self, block=None, value=None, lowlimit=None, highlimit=None, maxwait=None, wait_all=False,
        ...
        context = WaitForControllerExceptionContext(WaitForControllerSuppressNotifyReconnectState(self.api,
                                                                                                  datetime.utcnow(),
                                                                                                  None))
        ...
        while True:
            ...
            try:
                ...
                # Notify the context that the pv is connected
                context.pv_connected()
