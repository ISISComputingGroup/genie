...
class WaitForController(object):
    ...
    def start_waiting(self, block=None, value=None, lowlimit=None, highlimit=None, maxwait=None, wait_all=False,
                      ...
                      mevents=None,
                      early_exit=lambda: False, silent=False):
        """
        ...
        Args:
            ...
            silent: Suppress confirmation and countdown output to the console (warning/error messages still sent)
        ...
        timeout_msg = ''
        if not silent:
            print("Start time: {}".format(datetime.now().time().strftime("%H:%M:%S")))
        ...
        if mevents is not None:
            if not (isinstance(mevents, NUMERIC_TYPE)):
                ...
                if not silent:
                    print('Waiting for {} million events {}'.format(mevents, timeout_msg))
        ...
        self._init_wait_time(seconds, minutes, hours, silent, timeout_msg)
    ...
    def _init_wait_time(self, seconds, minutes, hours, silent=False, timeout_msg=""):
