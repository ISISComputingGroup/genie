...
class WaitForControllerState:
    ...
    """
    def __init__(self, last_notification_time, last_exception):
...
class WaitForControllerNotifyExceptionState(WaitForControllerState):
    ...
    def handle_exception_notification(self, exception):
        print("Exception in waitfor loop: {}: {}".format(exception.__class__.__name__, exception))
        self.context.transition_to(WaitForControllerSuppressNotifyExceptionState(datetime.utcnow(), exception))

    def pv_connected(self):
        self.context.transition_to(WaitForControllerNotifyReconnectState(datetime.utcnow(), self._last_exception))
...
class WaitForControllerSuppressNotifyExceptionState(WaitForControllerState):
    ...
    def handle_exception_notification(self, exception):
        ...
        if minutes_since_last_notification >= MINUTES_UNTIL_RENOTIFICATION_OF_EXCEPTION:
            self.context.transition_to(WaitForControllerNotifyExceptionState(self._last_notification_time, exception))
        if isinstance(exception, type(self._last_exception)):
            self.context.transition_to(WaitForControllerNotifyExceptionState(self._last_notification_time, exception))

    def pv_connected(self):
        self.context.transition_to(WaitForControllerNotifyReconnectState(self._last_notification_time,
...
class WaitForControllerNotifyReconnectState(WaitForControllerState):
    ...
    def handle_exception_notification(self, exception):
        self.context.transition_to(WaitForControllerNotifyExceptionState(self._last_notification_time,
                                                                         self._last_exception))

    def pv_connected(self):
        print("PV reconnected")
        self.context.transition_to(WaitForControllerSuppressNotifyReconnectState(self._last_notification_time,
                                                                                 self._last_exception))
...
class WaitForController(object):
    ...
    def start_waiting(self, block=None, value=None, lowlimit=None, highlimit=None, maxwait=None, wait_all=False,
        ...
        context = WaitForControllerExceptionContext(WaitForControllerSuppressNotifyReconnectState(datetime.utcnow(),
                                                                                                  None))
