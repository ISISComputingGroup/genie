...
# forward declare dummy API as cannot
# import from genie_python.genie_epics_api due to circular
# dependency
class API(object):
    ...
    def block_exists(self, block: str) -> bool:
        return True
    ...
    def get_block_value(
        self, name: str | None, to_string: bool = False, attempts: int = 3
    ) -> float:
        return 1.0
...
class WaitForController(object):
    ...
    def start_waiting(
        ...
        if block is not None:
            ...
            block = cast(str, self.api.correct_blockname(block, add_prefix=False))
    ...
    def _get_block_limits(
        self, value: WAITFOR_VALUE, lowlimit: float | None, highlimit: float | None
    ) -> tuple[WAITFOR_VALUE | None, WAITFOR_VALUE | None]:
        low = None
        high = None
        if value is not None:
            low = high = value
        if isinstance(lowlimit, float):
            low = lowlimit
        if isinstance(highlimit, float):
            high = highlimit
