...
class WaitForController(object):
    ...
    def start_waiting(self, block=None, value=None, lowlimit=None, highlimit=None, maxwait=None, wait_all=False,
        ...
        self.api.logger.log_info_msg("Start time: {}".format(datetime.now().time().strftime("%H:%M:%S")))
        ...
        while True:
            ...
            try:
                ...
                if len(res) == 0:
                    ...
                    self.api.logger.log_info_msg("End time: {}".format(datetime.now().time().strftime("%H:%M:%S")))
    ...
    def _init_wait_block(self, block, value, lowlimit, highlimit, quiet=False, timeout_msg=""):
        self.block = block
        ...
        if self.low == self.high:
            self._print_if_not_quiet('Waiting for {0}={1}{2}'.format(str(block), str(self.low), timeout_msg), quiet)
        else:
            self._print_if_not_quiet('Waiting for {0} (lowlimit={1}, highlimit={2}){3}'.format(str(block), str(self.low),
