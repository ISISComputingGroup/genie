...
class WaitForControllerState(object, metaclass=ABCMeta):
    ...
    def __init__(
        self, api: any, last_notification_time: any, last_exception: any, context: any = None
    ) -> None:
        """
...
class WaitForController(object):
    ...
    def wait_for_runstate(
        self, state: any, maxwaitsecs: int = 3600, onexit: bool = False, quiet: bool = False
    ) -> None:
        """
    ...
    def _init_wait_time(
        self, seconds: int, minutes: int, hours: int, quiet: bool = False, timeout_msg: str = ""
    ) -> None:
    ...
    def _waiting_for_time(self) -> bool:
        if (self.start_time is None) or (self.time_delta is None):
            raise Exception(
                    "start_time and time_delta must not be None"
                )
        else:
