...
class WaitForController(object):
    ...
    def start_waiting(self, block=None, value=None, lowlimit=None, highlimit=None, maxwait=None, wait_all=False,
        ...
        Args:
            ...
            events: number of events to wait
        ...
        if events is not None:
            if not (isinstance(events, NUMERIC_TYPE)):
                raise Exception("Invalid value entered for events")
            else:
                print('Waiting for {} events {}'.format(events, timeout_msg))
        if mevents is not None:
            if not (isinstance(mevents, NUMERIC_TYPE)):
                raise Exception("Invalid value entered for mevents")
            else:
                print('Waiting for {} millions of events {}'.format(mevents, timeout_msg))
        ...
        while True:
            ...
            try:
                ...
                if events is not None:
                    res.append(self.api.dae.get_events() < events)
                if mevents is not None:
