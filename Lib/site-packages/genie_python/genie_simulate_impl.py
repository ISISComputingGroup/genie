...
class Waitfor(object):
    ...
    def start_waiting(self, block=None, value=None, lowlimit=None, highlimit=None, maxwait=None, wait_all=None,
                      seconds=None, minutes=None, hours=None, time=None, frames=None, raw_frames=None, uamps=None,
                      mevents=None, early_exit=None, silent=False):
        # from https://stackoverflow.com/questions/582056/getting-list-of-parameter-names-inside-python-function
        frame = inspect.currentframe()
        args, _, _, values = inspect.getargvalues(frame)
        wait_str = ['{}={}'.format(arg, values[arg]) for arg in args if values[arg] is not None and arg != "self"]
        print('Waiting for {}'.format(', '.join(wait_str)))

    def wait_for_runstate(self, state, maxwaitsecs=3600, onexit=False, silent=False):
        if onexit:
...
class Dae(object):
    ...
    def integrate_spectrum(self, spectrum, period=1, t_min=None, t_max=None):
        ...
        Returns:
            float: integral of the spectrum (neutron counts) which is 1 proporiontal to the width requested
        """
...
class API(object):
    def __init__(self, pv_prefix=None, globs=None, strict_block=True):
        ...
        Args:
            ...
            globs: globals
            strict_block: if True will throw an exception if setting a block that doesn't exist, otherwise
    ...
    def get_runcontrol_settings(self, name):
        return self.block_dict[name]['runcontrol'], self.block_dict[name]['lowlimit'], self.block_dict[name]['highlimit']
